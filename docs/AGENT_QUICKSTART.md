...
One-page playbook for Claude / Cursor / Codex with Codehelper MCP.
...
1. Default advertise is **focused** (~12 tools). Override with `CODEHELPER_TOOL_PROFILE=full` when you need specialists listed. Use `core` for locate/plan-only sessions.
2. First call: `kickoff task="…"` (or `orchestrate` if local orchestration is on). Prefer composites over chaining query→context→impact (`investigate` for that fuse).
3. Paste a `next_queries` line next — do not invent tools.
4. Before edit: `change_kit target=<exact>` → `apply_patch_workspace_file` → `verify` → `finish_check` (diagnostics/review_diff by name if needed).
5. On arg errors: read the error text once, fix the **key name**, retry once — do not switch to Grep first.

## Claude Code

1. Trust the IDE-generated Codehelper agent instructions — MCP first, built-ins after empty/error.
...
| `task is required` / `recovery_hint=CHECK_INPUT` from `kickoff` | Pass `task=` (aliases `query=`/`q=`/`prompt=`/`request=` accepted with a correction note) |
