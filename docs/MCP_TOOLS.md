...
**Tool count:** 63 (see `internal/mcpsvc/toolcatalog.go` for the canonical grouped list).
...
**Tool profiles:** Default advertise is **focused** (~12 composite entry points). Hidden tools stay callable by name; `project_context` still returns the full catalog.

| Profile | How | Advertises |
|---|---|---|
| `core` | `CODEHELPER_TOOL_PROFILE=core` | ~8 conceptual slots (project/search/understand/impact/change/check/browser/workflow) |
| `focused` (**default**) | unset, `CODEHELPER_TOOL_PROFILE=focused`, `CODEHELPER_MINIMAL_TOOLS=1`, or `codehelper config project --minimal on` | ~12: slots + kickoff/orchestrate/investigate composites + apply_patch + finish_check |
| `full` | `CODEHELPER_TOOL_PROFILE=full` or `CODEHELPER_MINIMAL_TOOLS=off` | Full catalog (~60) |

Conceptual map: **project**→`project_context`, **search**→`query`, **understand**→`context`/`investigate`, **impact**→`impact`, **change**→`change_kit`, **check**→`verify`/`finish_check`, **browser**→`browser`, **workflow**→`kickoff`/`orchestrate`. Prefer composites over chaining primitives. See [PERMISSIONS.md](public/PERMISSIONS.md) and [HTTP_SECURITY.md](public/HTTP_SECURITY.md).
...
| `query` | Ranked symbol search (BM25 + trigram + call-graph centrality + synonym expansion). Production/app defs demote sample/test/fixture/style noise. Optional `group=<id>` fans out across workspace-group member `graph.db` files (`group_query` hits + `repo` per match; optional `path=`). |
| `scout` | Reuse-first search: ranked candidates, `usage_of_top` call site, `impact_of_top`, optional `collision_note`. |
| `context` | One symbol: definition excerpt, callers, callees, `blast_radius` (incl. call-edge `provenance` when present). Pass `path=` on name collisions. |
...
| `ast_query` | Tree-sitter structural search (~23 langs incl. SFC script bodies: svelte/vue/astro/mdx; parallel file scan). |
...
| `impact` | Blast radius; **default `direction=upstream`** (who uses this). Opt-in `downstream` for deps. Class hubs self-only on downstream auto-retry upstream. Inheritance/mixin densify (PHP/Ruby/C#/GDScript/Nest/Express) feeds inbound callers; still read `provenance` / confidence on sparse stacks. |
...
| `rename_symbol` | Rename across references (graph-first; merges LSP refs when available). |
| `lsp` | Optional hover/definition/references/rename/implementations via gopls/tsserver/pyright; graph fallback when missing. See [LSP.md](public/LSP.md). |
...
| `verify` | Run lint/build/test (argv default). Prefer plain strings (`"go test ./..."`); JSON arrays of tokens also accepted and Sprint-mangled `[cmd …]` blobs are recovered with a correction note. `exec_mode=shell` is off by default (`verify_allow_shell`); when enabled, **every** pipeline/compound executable must be allowlisted and redirects/newlines are rejected — the allowlist is not first-token-only. |
...
[browser-ui-eval.md](public/browser-ui-eval.md).
