...
pub fn dequantize_row_q8(q8: &Q8Matrix, row: u32, row_out: &mut [f32]) {
    ...
    let block = q8.block as usize;
    let mut j = 0usize;
    while j < q8.cols as usize {
        let idx = wbase + j;
        let scale = q8.scales[idx / block].to_f32() * awq_inv;
        // Scales are over the flat buffer — stop at the flat scale-block boundary, not
        // `j + block` in column space (wrong when `cols` is not a multiple of `block`).
        let flat_end = ((idx / block) + 1) * block;
        let row_end = (wbase + q8.cols as usize).min(flat_end);
        let block_len = row_end - idx;
        dequantize_q8_block(
            scale,
            &q8.packed[idx..idx + block_len],
            &mut row_out[j..j + block_len],
        );
        j += block_len;
    }
}
