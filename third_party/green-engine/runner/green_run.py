...
def main():
    ...
    ap.add_argument(
        "--ctx",
        type=int,
        default=int(os.environ.get("GE_CTX", "4096")),
        help="context window (KV budget grows with this; default GE_CTX or 4096)",
    )
    ap.add_argument(
        "--gpu-layers",
        type=int,
        default=int(os.environ.get("GE_GPU_LAYERS", "0")),
        help="layers to offload to GPU (default GE_GPU_LAYERS or 0=CPU; set e.g. 99 for full offload)",
    )
    ...
    if args.model and not os.path.isfile(os.path.expanduser(args.model)):
        sys.exit(
            f"green-run: model file not found: {args.model}\n"
            "  ge pull <hf-repo>  or pass an existing .gguf path.\n"
            "  Native .green packages are not supported yet."
        )
    ...
    try:
        ...
        sys.exit(
            ...
            "  Or put llama-cli on PATH for `ge run` native path."
        )
