...
def _chat_venv_python() -> Path:
    win = GE_HOME / "chat-venv" / "Scripts" / "python.exe"
    if win.is_file():
        return win
...
def _embed_venv_python() -> Path:
    win = GE_HOME / "embed-venv" / "Scripts" / "python.exe"
    if win.is_file():
        return win
...
def _pkill_pattern(pattern: str) -> None:
    if sys.platform == "win32":
        # Best-effort: terminate python processes whose command line matches.
        try:
            ps = (
        ...
        try:
            ...
            for line in out.stdout.splitlines():
                ...
                if needle not in line:
                    continue
                # Prefer exact :port before whitespace (avoid :87670 matching :8767)
                parts = line.split()
                addr = next((p for p in parts if p.endswith(needle) or re.search(rf":{port}$", p)), None)
                if not addr:
                    continue
                try:
                    return int(parts[-1])
...
def _build_status() -> dict[str, Any]:
    ge = _find_ge_bin()
    gc = _find_greencompress()
    embed_venv = _embed_venv_python().is_file()
    chat_venv = _chat_venv_python().is_file()
    models = _list_models()
    chat_model = ""
    chat_log = _log_file("chat")
    if chat_log.is_file():
