...
pub struct QuantMat {
    ...
    pub in_dim: usize,
    pub out_dim: usize,
    /// Packed bytes (or raw f32 LE for GGML_F32).
    pub packed: Vec<u8>,
}
...
impl QuantMat {
    ...
    pub fn nbytes_packed(ggml_type: u32, nelems: usize) -> Result<usize, GgufLoadError> {
        Ok(match ggml_type {
            GGML_F32 => nelems * 4,
            GGML_F16 => nelems * 2,
            GGML_Q4_0 => ((nelems + Q4_0_BLOCK - 1) / Q4_0_BLOCK) * Q4_0_BYTES,
            GGML_Q8_0 => ((nelems + Q8_0_BLOCK - 1) / Q8_0_BLOCK) * Q8_0_BYTES,
            GGML_Q4_K => ((nelems + QK_K - 1) / QK_K) * Q4_K_BYTES,
            GGML_Q6_K => ((nelems + QK_K - 1) / QK_K) * Q6_K_BYTES,
            other => {
