...
**Green-quant weights (Q4/Q8 + repair):** streaming matvec/matmul without full reconstruct is
implemented in `engine-core` (`--features green`) via greencompress CPU paths
(`GreenTensor::matvec` / `GreenExpert::apply_ffn` / `apply_attn_weight`). This crate stays f32
expert ABI until a CUDA toolkit + device dequant path exists.
