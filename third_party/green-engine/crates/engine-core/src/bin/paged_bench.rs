...
//!   GE_SLOT_POOL=per|global|ab            (default per; ab = PerLayer then Global A/B)

use engine_core::{
    dense_provider, CpuBackend, PagedFormat, PagedWeightStore, SlotBudget, Trace,
    WeightStore,
};
...
fn slot_modes() -> Vec<(&'static str, SlotBudget)> {
    let cap = env("GE_CAP", 24);
    match std::env::var("GE_SLOT_POOL")
        .unwrap_or_else(|_| "per".into())
        .to_ascii_lowercase()
        .as_str()
    {
        "global" | "shared" => vec![("global", SlotBudget::global(cap))],
        "ab" | "a/b" | "compare" => vec![
            ("per", SlotBudget::per_layer(cap)),
            ("global", SlotBudget::global(cap)),
        ],
        _ => vec![("per", SlotBudget::per_layer(cap))],
    }
}
...
fn main() {
    ...
    for (pool_label, budget) in modes {
        println!(
            "  pool={pool_label} ({:?})  slots/layer={}",
            budget.pool, budget.slots_per_layer
        );
        ...
        for fmt in [PagedFormat::F32, PagedFormat::Q8Ch, PagedFormat::Q4Ch] {
            let path = std::env::temp_dir().join(format!("ge_paged_bench_{}_{:?}_{}.bin", std::process::id(), fmt, pool_label));
            PagedWeightStore::create(&path, &store, fmt).expect("write paged store");
            let paged = PagedWeightStore::open_budget(&path, budget).expect("open paged store");
            ...
            for t in 0..tokens {
                for l in 0..layers {
                    ...
                    // Routing-aware hot-set: feed the transition predictor after the layer's set is known.
                    paged.observe_routing(l, chosen, Some(&gates));
    ...
    println!("  => Compression spectrum, all paged (RAM = per-layer cap, not whole model):");
