...
//! resident set here is a hard, explicit app-managed slot budget — the knob that trades RAM for
//! cold-read traffic, which the engine's scheduler/predictor then minimizes. Budget may be sized
//! in slots or bytes (GiB↔slots); pool policy is per-layer or a shared global pool (#25294 step 1).
...
/// How resident expert slots are partitioned under a fixed RAM budget.
#[derive(Clone, Copy, Debug, PartialEq, Eq, Default)]
pub enum SlotPool {
    /// Hard cap of `slots_per_layer` experts **per layer** (legacy).
    #[default]
    PerLayer,
    /// Shared pool of `slots_per_layer · layers` slots across all layers — same peak RAM as
    /// [`PerLayer`](SlotPool::PerLayer), but cold layers yield slots to hot ones.
    Global,
}

/// App-managed MoE resident-set budget (llama.cpp #25294 port step 1 — sizing only, not O_DIRECT).
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct SlotBudget {
    /// Nominal slots per layer. Total RAM ceiling = `slots_per_layer · layers · expert_bytes`
    /// under both pool modes.
    pub slots_per_layer: usize,
    pub pool: SlotPool,
}

impl SlotBudget {
    pub fn per_layer(slots: usize) -> Self {
        SlotBudget {
            slots_per_layer: slots.max(1),
            pool: SlotPool::PerLayer,
        }
    }

    pub fn global(slots_per_layer: usize) -> Self {
        SlotBudget {
            slots_per_layer: slots_per_layer.max(1),
            pool: SlotPool::Global,
        }
    }

    /// Convert a byte/GiB-class budget into slots: `floor(bytes / (stride · layers))` per layer.
    pub fn from_bytes(bytes: u64, expert_stride: u64, layers: usize, pool: SlotPool) -> Self {
        let layers = layers.max(1) as u64;
        let stride = expert_stride.max(1);
        let total_slots = (bytes / stride).max(1);
        let per = (total_slots / layers).max(1) as usize;
        SlotBudget {
            slots_per_layer: per,
            pool,
        }
    ...
    /// Parse `Ns` / bare int (slots), or `NM`/`NG`/`NMiB`/`NGiB` byte budgets.
    /// Examples: `"8"`, `"8s"`, `"32M"`, `"0.5G"`.
    pub fn parse(spec: &str, expert_stride: u64, layers: usize, pool: SlotPool) -> Option<Self> {
        let s = spec.trim();
        if s.is_empty() {
            return None;
        }
        ...
        if let Some(rest) = lower.strip_suffix('s') {
            ...
            return Some(SlotBudget {
                slots_per_layer: n.max(1),
                pool,
            });
        }
        let (num, mult) = if let Some(r) = lower.strip_suffix("gib") {
            (r.trim(), 1024u64 * 1024 * 1024)
        } else if let Some(r) = lower.strip_suffix("mib") {
            (r.trim(), 1024u64 * 1024)
        } else if let Some(r) = lower.strip_suffix('g') {
            (r.trim(), 1024u64 * 1024 * 1024)
        } else if let Some(r) = lower.strip_suffix('m') {
            (r.trim(), 1024u64 * 1024)
        } else {
            let n: usize = lower.parse().ok()?;
            return Some(SlotBudget {
                slots_per_layer: n.max(1),
                pool,
            });
        };
        let bytes = if num.contains('.') {
        ...
        Some(Self::from_bytes(bytes, expert_stride, layers, pool))
    }
}
...
pub trait ExpertProvider {
    ...
    /// Optional: record routed experts (+ router gates) for reuse-distance hot-set eviction.
    fn observe_routing(&self, _layer: usize, _experts: &[u16], _gates: Option<&[f32]>) {}
}
...
/// Per-layer bounded expert cache. Eviction is **reuse-distance** (Engine `Eviction::ReuseDistance`):
/// score residents by the token-transition predictor from the last routed set, tie-break by LRU.
/// Pure LFU/LFRU regresses on load-balanced OLMoE; reuse-distance raises hit rate under the same
/// per-layer cap (measured: +3–8 pp vs LRU at cap 8–10). Call [`PagedWeightStore::observe_routing`]
/// after each layer's routed set so the predictor stays causal.
///
/// Also tracks a tiny layer-ahead model (`trans_lay`): after observing layer L, returns the top-1
/// predicted expert for L+1 so the store can prefetch it (Engine `Prefetch::LayerAhead`, budget=1).
struct PagedCache {
    per_layer: Vec<HashMap<u16, Entry>>,
    clock: u64,
    cap: usize,  // experts resident PER LAYER (or per-layer share of a global pool)
    /// Total slots when [`SlotPool::Global`]: `cap * layers`.
    global_cap: usize,
    pool: SlotPool,
    peak: usize, // peak total resident experts across all layers
    trans: Vec<TransitionMatrix>,
    prev: Vec<Vec<u16>>,
    /// Cross-layer transitions: experts at L → experts at L+1 (same token).
    trans_lay: Vec<TransitionMatrix>,
    /// Last observed (layer, experts) in the current token sweep (for causal `trans_lay` updates).
    lay_prev: Option<(usize, Vec<u16>)>,
    scratch: Vec<f64>,
}

impl PagedCache {
    fn new(budget: SlotBudget, layers: usize, n_experts: usize) -> Self {
        ...
        PagedCache {
            ...
            pool: budget.pool,
            peak: 0,
            trans: (0..layers).map(|_| TransitionMatrix::new(n_experts)).collect(),
            prev: (0..layers).map(|_| Vec::new()).collect(),
            trans_lay: (0..layers).map(|_| TransitionMatrix::new(n_experts)).collect(),
            lay_prev: None,
    ...
    fn make_room_global(&mut self) {
        if self.total_resident() < self.global_cap {
            return;
        }
    ...
    /// Record routed set for reuse-distance eviction; return `(next_layer, top1_expert)` to prefetch
    /// when the layer-ahead predictor has mass (empty until the first L→L+1 observation).
    fn observe(&mut self, layer: usize, experts: &[u16], gates: Option<&[f32]>) -> Option<(usize, u16)> {
        ...
        // Causal cross-layer update: previous layer in this token → current.
        ...
        // Tiny next-layer prefetch: top-1 predicted expert for L+1.
...
mod slot_budget_tests {
    ...
    #[test]
    fn slot_budget_parse_slots_and_bytes() {
        let b = SlotBudget::parse("8s", 1024, 16, SlotPool::Global).unwrap();
        assert_eq!(b.slots_per_layer, 8);
        assert_eq!(b.pool, SlotPool::Global);
        // 32 MiB / (1 MiB stride · 16 layers) = 2 slots/layer
        let b = SlotBudget::from_bytes(32 * 1024 * 1024, 1024 * 1024, 16, SlotPool::PerLayer);
        assert_eq!(b.slots_per_layer, 2);
    }
}
...
impl PagedWeightStore {
    ...
    pub fn open<P: AsRef<Path>>(path: P, ram_experts: usize) -> io::Result<Self> {
        Self::open_budget(path, SlotBudget::per_layer(ram_experts))
    }

    /// Open with an explicit app-managed [`SlotBudget`] (slots or GiB-derived; per-layer or global pool).
    pub fn open_budget<P: AsRef<Path>>(path: P, budget: SlotBudget) -> io::Result<Self> {
        ...
        Ok(PagedWeightStore {
            ...
            cache: Mutex::new(PagedCache::new(budget, layers, experts)),
    ...
    /// Hard cap on resident RAM: `slots_per_layer · layers · expert_bytes` (both pool modes).
    pub fn resident_cap_bytes(&self) -> u64 {
        let c = self.cache.lock().unwrap();
        c.global_cap as u64 * self.stride as u64
    }

    pub fn slot_pool(&self) -> SlotPool {
        self.cache.lock().unwrap().pool
    }
...
impl ExpertProvider for PagedWeightStore {
    ...
    fn observe_routing(&self, layer: usize, experts: &[u16], gates: Option<&[f32]>) {
        PagedWeightStore::observe_routing(self, layer, experts, gates);
    }
}
...
impl PagedWeightStore {
    ...
    /// Record the routed expert set (and optional router gate weights) for this layer.
    /// Enables reuse-distance eviction on later tokens — routing-aware hot-set (WiSP / Engine
    /// `Eviction::ReuseDistance`). Gates weight the transition mass when provided (router-prob).
