...
impl TransitionMatrix {
    ...
    /// Router-prob weighted update: `gates[k]` scales the co-occurrence mass for `cur[k]`.
    /// Falls back to uniform `update` when `gates` is empty or length-mismatched.
    pub fn update_weighted(&mut self, prev: &[u16], cur: &[u16], gates: &[f32]) {
        if gates.len() != cur.len() || gates.is_empty() {
            self.update(prev, cur);
            return;
        }
        for &i in prev {
            let row = (i as usize) * self.experts;
            for (k, &j) in cur.iter().enumerate() {
                self.counts[row + j as usize] += gates[k] as f64;
            }
        }
    }
}
