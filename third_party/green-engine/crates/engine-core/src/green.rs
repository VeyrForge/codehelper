...
//!
//! A [`GreenTensor`] holds the Q8/Q4 base plus an optional [`Repair`]. Prefer
//! [`GreenTensor::matvec`] / [`GreenTensor::matmul`] (and [`GreenExpert::apply_ffn`] /
//! [`apply_attn_weight`]) which stream per-row dequant + repair without materializing the full
//! matrix. Full reconstruct (`to_f32_vec` / `reconstruct_into`) remains for the page-in / decode
//! cache path. The compression itself (SVD/sparse fit) is Green Compress's job; the engine stores
//! and applies it.
...
use greencompress::q4::{dequantize_q4, matmul_q4_repaired, quantize_q4, reconstruct as reconstruct_q4};
use greencompress::q8::{dequantize_q8, dequantize_row_q8, q8_row_spin, quantize_q8, reconstruct_q8};
use greencompress::repair::{apply_repair_matmul, fit_repair};
use greencompress::simd::saxpy_accumulate;
...
impl GreenTensor {
    ...
    /// `out = x @ W` without materializing the full dequantized weight matrix.
    ///
    /// `x` is length `rows` (in_dim), `out` is length `cols` (out_dim). Uses Green Compress
    /// per-row Q8/Q4 dequant + saxpy, then applies low-rank/sparse repair in matmul space.
    ///
    /// TODO: Qn / Q4_K / fused-weight-cache layouts are not stored in [`GreenTensor`] yet — if
    /// those appear in a package, fall back to [`Self::to_f32_vec`] + dense matvec until Base grows.
    pub fn matvec(&self, x: &[f32], out: &mut [f32]) {
        assert_eq!(x.len(), self.rows as usize, "GreenTensor::matvec: x len != rows");
        assert_eq!(out.len(), self.cols as usize, "GreenTensor::matvec: out len != cols");
        let y = self.matmul_inner(x, 1);
        out.copy_from_slice(&y);
    }

    /// Batch GEMM-like apply: `out[b] = activations[b] @ W` for `b in 0..batch`.
    ///
    /// `activations` is row-major `[batch, rows]`, `out` is row-major `[batch, cols]`.
    pub fn matmul(&self, activations: &[f32], batch: usize, out: &mut [f32]) {
        let rows = self.rows as usize;
        let cols = self.cols as usize;
        assert_eq!(
            activations.len(),
            batch * rows,
            "GreenTensor::matmul: activations len != batch*rows"
        );
        assert_eq!(out.len(), batch * cols, "GreenTensor::matmul: out len != batch*cols");
        let y = self.matmul_inner(activations, batch);
        out.copy_from_slice(&y);
    }

    fn matmul_inner(&self, activations: &[f32], batch: usize) -> Vec<f32> {
        let act = Matrix {
            rows: batch as u32,
            cols: self.rows,
            data: activations.to_vec(),
        };
        match &self.base {
            // Q8: per-row dequant (+ optional row_spin) + saxpy, then full repair in matmul
            // space. Matches reconstruct_q8; avoids matmul_q8_repaired's sparse/spin edge cases.
            Base::Q8(q8) => {
                ...
                let mut out = Matrix {
                    rows: batch as u32,
                    cols: q8.cols,
                    data: vec![0.0; batch * cols],
                };
                let mut w_row = vec![0.0f32; cols];
                for b in 0..batch {
                    let out_row = &mut out.data[b * cols..(b + 1) * cols];
                    let abase = b * k_rows;
                    for k in 0..q8.rows {
                        dequantize_row_q8(q8, k, &mut w_row);
                        ...
                        saxpy_accumulate(act.data[abase + k as usize], &w_row, out_row);
                    }
                }
                if let Some(repair) = self.repair.as_ref() {
                    apply_repair_matmul(&mut out, &act, repair, q8.rows, q8.cols, None);
                }
                out.data
            }
            Base::Q4(q4) => matmul_q4_repaired(&act, q4, self.repair.as_ref(), None).data,
        }
    }
...
impl GreenExpert {
    ...
    /// SwiGLU FFN on compressed gate/up/down: `y = down(silu(x @ gate) ⊙ (x @ up))`.
    ///
    /// Does **not** reconstruct the three weight matrices to f32; each projection streams
    /// Green-quant rows. `x` and `out` are length `hidden` (`gate.rows` / `down.cols`).
    pub fn apply_ffn(&self, x: &[f32], out: &mut [f32]) {
        let hidden = self.gate.rows as usize;
        let inter = self.gate.cols as usize;
        assert_eq!(x.len(), hidden, "GreenExpert::apply_ffn: x len != hidden");
        assert_eq!(out.len(), hidden, "GreenExpert::apply_ffn: out len != hidden");
        assert_eq!(self.up.rows as usize, hidden);
        assert_eq!(self.up.cols as usize, inter);
        assert_eq!(self.down.rows as usize, inter);
        assert_eq!(self.down.cols as usize, hidden);

        let mut g = vec![0.0f32; inter];
        let mut u = vec![0.0f32; inter];
        self.gate.matvec(x, &mut g);
        self.up.matvec(x, &mut u);
        for j in 0..inter {
            g[j] = silu(g[j]) * u[j];
        }
        self.down.matvec(&g, out);
    }
}

/// Apply one attention projection weight (`W_q` / `W_k` / `W_v` / `W_o` as a [`GreenTensor`]).
///
/// `x`: `[in_dim] = w.rows`, `out`: `[out_dim] = w.cols`. Same streaming path as FFN projections.
pub fn apply_attn_weight(w: &GreenTensor, x: &[f32], out: &mut [f32]) {
    w.matvec(x, out);
}

/// Batch attention projection: `activations` row-major `[batch, in]`, `out` `[batch, out]`.
pub fn apply_attn_weight_batch(
    w: &GreenTensor,
    activations: &[f32],
    batch: usize,
    out: &mut [f32],
) {
    w.matmul(activations, batch, out);
}
...
mod tests {
    ...
    #[test]
    fn green_matvec_matches_full_reconstruct_q8_and_q4() {
        let (rows, cols) = (64usize, 48usize);
        let w = structured(rows, cols, 19);
        let mut seed = 42u64;
        ...
        for (tag, gt) in [
            ("q8+repair", GreenTensor::compress(&w, rows, cols, 64, 8, 0.01)),
            ("q4+repair", GreenTensor::compress_q4(&w, rows, cols, 64, 8, 0.01)),
        ] {
            let full = gt.to_f32_vec();
            let mut want = vec![0.0f32; cols];
            crate::tensor::matvec(&x, &full, rows, cols, &mut want);
            ...
            assert!(
                drift < 1e-5,
                "{tag}: streaming matvec vs reconstruct drift {drift}"
            );
        }
    }

    #[test]
    fn green_apply_ffn_matches_reconstructed_swiglu() {
        use crate::tensor::swiglu_ffn;
        use crate::weights::ExpertWeights;

        let (h, inter) = (32usize, 64usize);
        let store = WeightStore::synthetic(1, 1, h, inter, false, 3);
        let ew = store.get(0, 0);
        let ge = GreenExpert::compress(ew, GreenBase::Q8, 64, 4, 0.005);
        ...
        let recon = ExpertWeights {
            hidden: h,
            inter,
            gate: Tensor::F32(ge.gate.to_f32_vec()),
            up: Tensor::F32(ge.up.to_f32_vec()),
            down: Tensor::F32(ge.down.to_f32_vec()),
        };
        let mut g = vec![0.0f32; inter];
        let mut u = vec![0.0f32; inter];
        ...
        swiglu_ffn(
            &x,
            &recon.gate.to_f32_vec(),
            &recon.up.to_f32_vec(),
            &recon.down.to_f32_vec(),
            h,
            inter,
            &mut g,
            &mut u,
        ...
        ge.apply_ffn(&x, &mut got);
        let drift = rel_l2(&want, &got);
        assert!(drift < 1e-5, "apply_ffn vs reconstruct+swiglu drift {drift}");
    }

    #[test]
    fn apply_attn_weight_matches_matvec() {
        let (rows, cols) = (40usize, 24usize);
        let w = structured(rows, cols, 5);
        let gt = GreenTensor::compress(&w, rows, cols, 64, 4, 0.01);
        let mut seed = 99u64;
        let x: Vec<f32> = (0..rows)
            .map(|_| {
                seed ^= seed >> 12;
