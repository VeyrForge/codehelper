...
func serveCmd() *cobra.Command {
	...
	c := &cobra.Command{
		...
		Long: "Starts the loopback HTTP server that exposes the Go agent loop to thin clients " +
			...
			"CODEHELPER_API_TOKEN is required for chat/tools (fail-closed when unset); /healthz stays public and slim.",
...
// validateServeLoopbackAddr rejects any --addr that is not an explicit
// loopback host, so `codehelper serve` can never accidentally bind beyond the
// local machine — it exposes the Go agent loop (LLM credentials, workspace
// read/write) behind CODEHELPER_API_TOKEN for mutating routes.
func validateServeLoopbackAddr(addr string) error {
	addr = strings.TrimSpace(addr)
	host, _, err := net.SplitHostPort(addr)
	if err != nil {
		return fmt.Errorf("serve binds loopback only; got %q", addr)
	}
	if host == "" || !mcpsvc.IsLoopbackHost(host) {
		return fmt.Errorf("serve binds loopback only; got %q", addr)
	}
	return nil
}
