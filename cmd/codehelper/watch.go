...
func watchCmd() *cobra.Command {
	...
	var daemonize, stopRequested, stopAll, statusOnly bool
	...
	c.Flags().BoolVar(&stopAll, "stop-all", false, "stop every live watch daemon recorded in the registry")
...
func stopDaemon(indexRoot string) error {
	...
	if err := signalWatchDaemon(pid); err != nil {
		return err
	}
	fmt.Printf("sent stop to watch daemon pid=%d\n", pid)
	return nil
}

// stopAllDaemons terminates every live watch daemon recorded for registry
// repos. Used to recover from accidental mass auto-watch after bulk analyze.
func stopAllDaemons() error {
	...
	for _, e := range reg.List() {
		...
		if rerr != nil || st == nil || st.PID <= 1 || !daemon.ProcessAlive(st.PID) {
			continue
		}
		...
		seen[st.PID] = struct{}{}
		if serr := signalWatchDaemon(st.PID); serr != nil {
			slog.Warn("watch --stop-all signal", "pid", st.PID, "root", e.RootPath, "err", serr)
			continue
		}
		fmt.Printf("sent stop to watch daemon pid=%d root=%s\n", st.PID, e.RootPath)
		stopped++
	}
	fmt.Printf("watch --stop-all: signaled %d daemon(s)\n", stopped)
	return nil
}
