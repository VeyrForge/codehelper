...
func projectsCmd() *cobra.Command {
	...
	c.AddCommand(projectsListCmd(), projectsCurrentCmd(), projectsForgetCmd(), projectsPruneCmd(),
		projectsGroupCmd(), projectsContractsCmd(), projectsSnapshotCmd(), projectsCrossQueryCmd())
	return c
}
...
func projectsListCmd() *cobra.Command {
	...
	c := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fmt.Println("Link siblings with `codehelper projects group set` (see docs/WORKSPACE_GROUPS.md).")
			return nil
		},
	}
