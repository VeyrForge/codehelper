...
Policy (CLI only — never via MCP):
  ...
  policy set --verify-allow-shell             permit exec_mode=shell (off by default; every compound exe allowlisted; no redirects)
...
func connectionsPolicySetCmd() *cobra.Command {
	...
	var allowGit, githubDisable, verifyAllowShell bool
	c := &cobra.Command{
		Use:   "set",
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if cmd.Flags().Changed("verify-allow-shell") {
				p.VerifyAllowShell = verifyAllowShell
			}
	...
	c.Flags().BoolVar(&verifyAllowShell, "verify-allow-shell", false, "permit exec_mode=shell for local verify (off by default; every pipeline/compound exe still allowlisted; redirects blocked; agents cannot enable)")
