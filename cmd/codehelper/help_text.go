...
Optional: --http ADDR for Streamable HTTP (host-less :port rewrites to 127.0.0.1;
always requires CODEHELPER_MCP_TOKEN or auto-gen ~/.codehelper/mcp_http.token).
Metrics via --metrics-addr or CODEHELPER_METRICS_ADDR. See docs/HTTP_SECURITY.md.

Tool profiles (advertise list only; hidden tools stay callable by name):
  default = focused (~12 conceptual + composite entry points)
  CODEHELPER_TOOL_PROFILE=core|focused|full
  full override: CODEHELPER_TOOL_PROFILE=full or CODEHELPER_MINIMAL_TOOLS=off
  focused aliases: CODEHELPER_MINIMAL_TOOLS=1, codehelper config project --minimal on
  slots: project/search/understand/impact/change/check/browser/workflow
  Prefer kickoff / investigate / orchestrate over chaining primitives.
