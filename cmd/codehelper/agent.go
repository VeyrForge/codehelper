...
func agentFinishCmd() *cobra.Command {
	var base string
	var verifyRan, verifyAbstained, allowClaimWithoutVerify, diagnosticsRan bool
	...
	c := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			hasEditProof := false
			if files, derr := gitutil.DiffAgainst(root, bref); derr == nil {
				hasEditProof = len(files) > 0
			} else if bref == "HEAD~1" {
				if files, derr2 := gitutil.DiffAgainst(root, "HEAD"); derr2 == nil {
					hasEditProof = len(files) > 0
				}
			}
			out := review.BuildFinishCheck(review.FinishCheckInput{
				...
				AllowClaimWithoutVerify: allowClaimWithoutVerify,
				HasEditProof:            hasEditProof,
				DiagnosticsRan:          diagnosticsRan,
				Release:                 rr,
			})
			b, _ := json.MarshalIndent(out, "", "  ")
			fmt.Println(string(b))
	...
	c.Flags().BoolVar(&allowClaimWithoutVerify, "allow-claim-without-verify", false, "explicit allow when verify abstained")
	c.Flags().BoolVar(&diagnosticsRan, "diagnostics-ran", false, "diagnostics was executed")
	return c
}
