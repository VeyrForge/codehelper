...
With no flags it prints the effective config. Flags:
  ...
  --minimal on|off     trim tools/list to the focused profile (~12 entry
                       points). Alias of focused tool profile; hidden tools
                       stay callable by name. Default advertise is already
                       focused — use CODEHELPER_TOOL_PROFILE=full (or
                       CODEHELPER_MINIMAL_TOOLS=off) for the full catalog.
                       Also: CODEHELPER_TOOL_PROFILE=core|focused|full
...
Toggling takes effect on the running MCP server without a restart.`,
	...
	c.Flags().StringVar(&minimalFlag, "minimal", "", "on|off — advertise focused tool profile in tools/list")
