...
// ModuleNodeID is moduleNodeID for callers outside this package that synthesize
// import edges after extraction (indexer path-alias expansion).
func ModuleNodeID(repoID, mod string) string { return moduleNodeID(repoID, mod) }

// EdgeID is edgeID for callers outside this package that synthesize edges after
// extraction; it keeps their ids byte-identical to parser-emitted ones.
func EdgeID(repoID, src, dst, kind string) string { return edgeID(repoID, src, dst, kind) }
