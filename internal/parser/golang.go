...
func ParseGo(ctx context.Context, repoID, relPath string, buf []byte) (*ParseResult, error) {
	...
	Walk(tree.RootNode(), func(n *sitter.Node) {
		...
		case "function_declaration":
			...
			// Gin/Echo/Fiber ALLCAPS + Chi/Beego path registrations → handler edges.
			extractGoHTTPRouteHandlers(n, buf, repoID, relPath, sym.ID, out)
...
func goTypeName(n *sitter.Node, buf []byte) string {
	...
	case "qualified_type":
		// controllers.UserController → UserController leaf
		if name := n.ChildByFieldName("name"); name != nil {
			return strings.TrimSpace(name.Content(buf))
		}
		if n.NamedChildCount() > 0 {
			return goTypeName(n.NamedChild(int(n.NamedChildCount())-1), buf)
		}
	}
	return ""
}
...
// goHTTPRouteMethods are Gin/Echo/Fiber/Chi/Beego (and net/http-ish) registration
// callees. Lowercase Get/Handle require a path-like string arg so cache.Get("k")
// does not invent handler edges.
var goHTTPRouteMethods = map[string]bool{
	"GET": true, "POST": true, "PUT": true, "DELETE": true, "PATCH": true,
	"OPTIONS": true, "HEAD": true, "CONNECT": true, "TRACE": true,
	"Any": true, "All": true, "Match": true, "Add": true,
	"Get": true, "Post": true, "Put": true, "Delete": true, "Patch": true,
	"Options": true, "Head": true, "Connect": true, "Trace": true,
	"Method": true, "MethodFunc": true, "Handle": true, "HandleFunc": true,
	"Mount": true, "Static": true, "StaticFS": true, "File": true, "FileFS": true,
	"RouteNotFound": true, "Router": true, "Handler": true, "NSRouter": true,
	"InsertFilter": true, "InsertFilterChain": true,
}

// goHTTPRouteMethodsLoose always emit handler args (ALLCAPS gin/echo/fiber + beego).
var goHTTPRouteMethodsLoose = map[string]bool{
	"GET": true, "POST": true, "PUT": true, "DELETE": true, "PATCH": true,
	"OPTIONS": true, "HEAD": true, "CONNECT": true, "TRACE": true,
	"Any": true, "All": true, "Static": true, "StaticFS": true, "File": true, "FileFS": true,
	"RouteNotFound": true, "Router": true, "Handler": true, "NSRouter": true,
	"InsertFilter": true, "InsertFilterChain": true,
}

// extractGoHTTPRouteHandlers finds r.GET("/x", Health) / chi Get("/x", Health) /
// beego.Router("/", &Ctrl{}) style registrations and emits calls edges to handlers.
func extractGoHTTPRouteHandlers(root *sitter.Node, buf []byte, repoID, relPath, fromSym string, out *ParseResult) {
	if root == nil || fromSym == "" || out == nil {
		return
	}
	Walk(root, func(n *sitter.Node) {
		...
		meth := goCallMethodName(n, buf)
		if !goHTTPRouteMethods[meth] {
			return
		}
		...
		if !goHTTPRouteMethodsLoose[meth] && !goHasRoutePathArg(args, buf) {
			return
		}
		emitGoRouteHandlerArgs(args, buf, repoID, relPath, fromSym, out)
	})
}

func goCallMethodName(call *sitter.Node, buf []byte) string {
	...
	case "selector_expression":
		if field := fn.ChildByFieldName("field"); field != nil {
			return strings.TrimSpace(field.Content(buf))
		}
	case "identifier":
		return strings.TrimSpace(fn.Content(buf))
	}
	return ""
}

func goHasRoutePathArg(args *sitter.Node, buf []byte) bool {
	if args == nil {
		return false
	}
	for i := 0; i < int(args.NamedChildCount()) && i < 3; i++ {
		c := args.NamedChild(i)
		if c == nil {
			continue
		}
		...
		case "interpreted_string_literal", "raw_string_literal":
			if goLooksLikeRoutePath(c.Content(buf)) {
				return true
			}
		}
	}
...
func goLooksLikeRoutePath(lit string) bool {
	s := strings.TrimSpace(lit)
	s = strings.Trim(s, "`\"")
	if s == "" {
		return false
	}
	// Gin/Echo/Chi paths are slash- or star-rooted; brace params are Chi/Echo-style.
	return strings.HasPrefix(s, "/") || strings.HasPrefix(s, "*") || strings.Contains(s, "{")
}

func emitGoRouteHandlerArgs(args *sitter.Node, buf []byte, repoID, relPath, fromSym string, out *ParseResult) {
	if args == nil {
		return
	}
	for i := 0; i < int(args.NamedChildCount()); i++ {
		emitGoRouteHandlerArg(args.NamedChild(i), buf, repoID, relPath, fromSym, out)
	}
}

func emitGoRouteHandlerArg(n *sitter.Node, buf []byte, repoID, relPath, fromSym string, out *ParseResult) {
	if n == nil {
		return
	}
	switch n.Type() {
	case "identifier":
		...
		if name == "" || !isCallableName(name) || goHTTPRouteMethods[name] {
			return
		}
		emitGoRouteHandlerCall(repoID, relPath, fromSym, name, out)
	case "selector_expression":
		// s.ListUsers / ctrl.Get — emit the method/field leaf.
		if field := n.ChildByFieldName("field"); field != nil {
			name := strings.TrimSpace(field.Content(buf))
			if name != "" && isCallableName(name) && !goHTTPRouteMethods[name] {
				emitGoRouteHandlerCall(repoID, relPath, fromSym, name, out)
			}
		}
	case "unary_expression":
		// &UserController{}
		emitGoRouteHandlerArg(n.ChildByFieldName("operand"), buf, repoID, relPath, fromSym, out)
	case "composite_literal":
		if typ := goTypeName(n.ChildByFieldName("type"), buf); typ != "" {
			emitGoRouteHandlerCall(repoID, relPath, fromSym, typ, out)
		}
	case "parenthesized_expression":
		if n.NamedChildCount() > 0 {
			emitGoRouteHandlerArg(n.NamedChild(0), buf, repoID, relPath, fromSym, out)
		}
	}
}

func emitGoRouteHandlerCall(repoID, relPath, fromSym, name string, out *ParseResult) {
