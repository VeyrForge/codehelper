...
`)
	...
	var sawExtends bool
	for _, e := range res.Edges {
		if e.Kind == types.RefKindInherits && strings.HasSuffix(e.TargetID, ":Resource") {
			sawExtends = true
		}
	}
	if !sawExtends {
		t.Fatal("expected extends Resource inherits edge")
	}
	for _, s := range res.Symbols {
		if s.Name == "add_obstacle" && s.ParentID != "CustomObstacleLibrary" {
			t.Fatalf("add_obstacle ParentID=%q want CustomObstacleLibrary", s.ParentID)
		}
	}
}

func TestParseGDScript_AsCastAndPercentNode(t *testing.T) {
	src := []byte(`class_name Player
extends CharacterBody2D

func _ready():
	var h = %HealthBar as HealthBar
	MatchConfig.reset_lobby()
`)
	res, err := parseGDScriptLite(context.Background(), "repo", "player.gd", src)
	if err != nil {
		t.Fatal(err)
	}
	var readyID string
	for _, s := range res.Symbols {
		if s.Name == "_ready" {
			...
			if s.ParentID != "Player" {
				t.Fatalf("_ready ParentID=%q want Player", s.ParentID)
			}
		}
	}
	if readyID == "" {
		t.Fatal("missing _ready")
	}
	reads := map[string]bool{}
	for _, e := range res.Edges {
		if e.Kind == types.RefKindReads && e.SourceID == readyID {
			reads[symrefName(e.TargetID)] = true
		}
	}
	for _, want := range []string{"HealthBar"} {
		if !reads[want] {
			t.Errorf("missing read %q; got %#v", want, reads)
		}
	}
}

func TestParseGDScript_FileStemParentID(t *testing.T) {
	src := []byte(`extends Node2D

func _ready():
	pass
`)
	res, err := parseGDScriptLite(context.Background(), "repo", "scripts/enemy.gd", src)
	if err != nil {
		t.Fatal(err)
	}
	for _, s := range res.Symbols {
		if s.Name == "_ready" {
			if s.ParentID != "enemy" {
				t.Fatalf("_ready ParentID=%q want enemy (file stem)", s.ParentID)
			}
			return
		}
	}
