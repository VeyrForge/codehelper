...
var astLanguages = map[string]func() *sitter.Language{
	...
	// Svelte/Vue/Astro SFCs have no dedicated grammar here — map to the TSX/JS
	// grammar and scan extracted <script> (and Astro frontmatter) bodies.
	"svelte": tsx.GetLanguage,
	"vue":    tsx.GetLanguage,
	"astro":  tsx.GetLanguage,
	"mdx":    tsx.GetLanguage,
}
...
type ASTScanner struct {
	...
	svelteMode bool // extract <script> bodies (Svelte/Vue) before querying
	astroMode  bool // extract frontmatter + <script> from .astro
	mdxMode    bool // query masked JS islands in .mdx
}
...
func NewASTScanner(language, pattern string) (*ASTScanner, error) {
	...
	return &ASTScanner{
		...
		svelteMode: langKey == "svelte" || langKey == "sveltekit" || langKey == "vue" || langKey == "vuejs",
		astroMode:  langKey == "astro",
		mdxMode:    langKey == "mdx",
	}, nil
}
...
func (s *ASTScanner) ScanFile(ctx context.Context, relPath string, buf []byte, out *[]ASTMatch, limit int) (err error) {
	...
	if s.astroMode {
		return s.scanAstroScripts(ctx, relPath, buf, out, limit)
	}
	if s.mdxMode {
		masked := mdxMaskNonJS(string(buf))
		return s.scanBuffer(ctx, relPath, []byte(masked), 0, out, limit)
	}
...
func (s *ASTScanner) scanAstroScripts(ctx context.Context, relPath string, buf []byte, out *[]ASTMatch, limit int) error {
	text := string(buf)
