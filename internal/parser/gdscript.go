...
func parseGDScriptLite(_ context.Context, repoID, relPath string, buf []byte) (*ParseResult, error) {
	...
	currentFuncIndent := -1
	// Prefer class_name / nested class; fall back to file stem so _ready on
	// scripts without class_name still carries ParentID for MCP disambiguation.
	className := gdScriptParentFallback(relPath)
	var classSymID string
	var pendingExtends []string
	emitExtends := func(fromID string, bases []string) {
		for _, base := range bases {
			base = strings.TrimSpace(base)
			if base == "" {
				continue
			}
			tgt := fmt.Sprintf("symref:%s:%s:%s", repoID, relPath, base)
	...
	for _, ln := range strings.Split(string(buf), "\n") {
		...
		for _, d := range gdDecls {
			...
			if d.kind == types.SymbolKindFunction {
				parent = className
				// Class-bound funcs as methods so ResolveSymrefs recv_type
				// (Enemy.take_hit) densifies inbound CallersOf like C#/C++.
				if parent != "" {
					kind = types.SymbolKindMethod
				}
			}
			sym := symbol(repoID, relPath, m[1], kind, line, line, "gdscript", "", parent)
			out.Symbols = append(out.Symbols, sym)
			out.Edges = append(out.Edges, containsEdge(repoID, relPath, sym.ID))
			if d.kind == types.SymbolKindClass {
				className = m[1]
				classSymID = sym.ID
				if len(pendingExtends) > 0 {
					emitExtends(classSymID, pendingExtends)
					pendingExtends = nil
				}
			}
			if d.kind == types.SymbolKindFunction {
		...
		// Top-level extends ClassName → inherits (prefer class_name source for impact).
		if !declMatched {
			if m := gdExtendsRe.FindStringSubmatch(ln); len(m) > 1 {
				base := m[1]
				if classSymID != "" {
					emitExtends(classSymID, []string{base})
				} else {
					pendingExtends = append(pendingExtends, base)
				}
			}
		}
		if declMatched {
	...
	// No class_name — attach pending extends to the file node so CallersOf still
	// sees inherits inbound on engine bases / sibling scripts.
	if len(pendingExtends) > 0 {
		emitExtends(fid, pendingExtends)
	}
	return out, nil
}
...
// gdScriptParentFallback returns a ParentID hint from the .gd file stem when
// the script has no class_name (common Godot pattern). PascalCase stems keep
// their casing; snake_case stems stay as-is so recv matches the path hint.
func gdScriptParentFallback(relPath string) string {
	base := relPath
	if i := strings.LastIndexAny(relPath, `/\`); i >= 0 {
		base = relPath[i+1:]
	}
	stem := strings.TrimSuffix(base, ".gd")
	stem = strings.TrimSuffix(stem, ".GD")
	if stem == "" || stem == base {
		return ""
	}
	return stem
...
// gdAsCastRe matches `x as ClassName` typed casts (common Godot pattern).
var gdAsCastRe = regexp.MustCompile(`\bas\s+([A-Z][A-Za-z0-9_]*)\b`)

// gdGetNodeRe matches get_node("Path/To") / get_node(%Unique) — last path segment
// when it looks like a PascalCase class/script name is rare; prefer %UniqueName.
var gdPercentNodeRe = regexp.MustCompile(`%([A-Z][A-Za-z0-9_]*)`)
...
func emitGDScriptCalls(repoID, relPath, fid, fromSym, ln string, out *ParseResult) {
	...
	for _, m := range gdAsCastRe.FindAllStringSubmatch(ln, -1) {
		emitRead(m[1], 0.75)
	}
	for _, m := range gdPercentNodeRe.FindAllStringSubmatch(ln, -1) {
		emitRead(m[1], 0.65)
	}
