...
type Request struct {
	...
	// deny-all — callers should pass DefaultVerifyAllowlist or a project list.
...
// ErrShellNotAllowed is returned when exec_mode=shell but the project has not
// opted in via verify_allow_shell (user CLI only).
var ErrShellNotAllowed = errors.New("shell mode requires project verify_allow_shell")
...
func runCommand(ctx context.Context, dir, cmdline string, mode ExecMode, allow []string, blockPol BlockPolicy, timeout time.Duration) (string, ExecStat, error) {
	...
	switch mode {
	case ExecShell:
		if !blockPol.AllowShell {
			stat.DurationS = time.Since(t0).Seconds()
			stat.ExitCode = -1
			return "", stat, fmt.Errorf("%w: set via `codehelper connections policy set --verify-allow-shell` (agents cannot enable shell)", ErrShellNotAllowed)
		}
		if blocked, reason := ShellLineBlocked(cmdline, blockPol); blocked {
			stat.DurationS = time.Since(t0).Seconds()
			stat.ExitCode = -1
			return "", stat, fmt.Errorf("%w: %s", ErrCommandBlocked, reason)
		}
		// Allowlist applies to EVERY pipeline/compound segment executable — not
		// only the first token. (`go test; curl|sh` must not pass when only go
		// is listed.) Agents cannot widen the list or enable shell.
		if len(allow) == 0 {
			stat.DurationS = time.Since(t0).Seconds()
			stat.ExitCode = -1
			return "", stat, fmt.Errorf("%w: empty allowlist (deny-all)", ErrCommandNotAllowed)
		}
		exes, ferr := shellSegmentExecutables(cmdline)
		if ferr != nil {
			stat.DurationS = time.Since(t0).Seconds()
			stat.ExitCode = -1
			if errors.Is(ferr, ErrShellLineUnsupported) || errors.Is(ferr, ErrUnclosedQuote) || errors.Is(ferr, ErrEmptyCommand) {
				return "", stat, fmt.Errorf("%w: %v", ErrCommandBlocked, ferr)
			}
			return "", stat, ferr
		}
		for _, exe := range exes {
			if !allowed(exe, allow) {
				stat.DurationS = time.Since(t0).Seconds()
				stat.ExitCode = -1
				return "", stat, fmt.Errorf("%w: %s (shell mode requires every pipeline/compound executable on the allowlist)", ErrCommandNotAllowed, exe)
			}
		}
	...
	default:
		...
		// Empty allowlist means deny-all (never allow-all). Callers that want
		// the default toolchain set must pass DefaultVerifyAllowlist via policy.
		if len(allow) == 0 {
			stat.DurationS = time.Since(t0).Seconds()
			stat.ExitCode = -1
			return "", stat, fmt.Errorf("%w: empty allowlist (deny-all)", ErrCommandNotAllowed)
		}
		if !allowed(argv[0], allow) {
			stat.DurationS = time.Since(t0).Seconds()
			stat.ExitCode = -1
...
// shellCommandFirst returns the first segment's executable for tests and
// diagnostics. Prefer shellSegmentExecutables for allowlist enforcement.
func shellCommandFirst(cmdline string) (string, error) {
	exes, err := shellSegmentExecutables(cmdline)
	if err != nil {
		return "", err
	}
	if len(exes) == 0 {
