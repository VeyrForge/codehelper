...
// DefaultRepo is the default GitHub owner/repo for public product releases.
// Private/dev builds can override with CODEHELPER_UPGRADE_REPO or upgrade --repo
// (e.g. VeyrForge/codehelper). Go module path remains github.com/VeyrForge/codehelper.
const DefaultRepo = "VeyrForge/codehelper"

// EnvUpgradeRepo overrides DefaultRepo when Options.GitHubRepo is empty.
const EnvUpgradeRepo = "CODEHELPER_UPGRADE_REPO"

// MaxDownloadBytes caps a single release asset download (fail closed).
var MaxDownloadBytes int64 = 512 << 20 // 512 MiB

// Options configures a release install.
type Options struct {
	GitHubRepo      string // "owner/repo"; empty uses CODEHELPER_UPGRADE_REPO then DefaultRepo
	Tag             string // "latest" or "v1.2.3"
	CurrentVersion  string // embedded main.Version; upgrade skipped if equal (unless Force)
	Force           bool
	AllowUnverified bool   // skip missing/unusable checksums.txt (explicit opt-in only)
...
func (o Options) repo() string {
	if s := strings.TrimSpace(o.GitHubRepo); s != "" {
		return s
	}
	if s := strings.TrimSpace(os.Getenv(EnvUpgradeRepo)); s != "" {
		return s
	}
	return DefaultRepo
}
...
func Upgrade(destExe string, replace func(tmpExe, destExe string) error, opts Options) error {
	...
	if resp.StatusCode != http.StatusOK {
		...
		return fmt.Errorf("GitHub API %s: %s â€” %s", resp.Status, relURL, strings.TrimSpace(string(b)))
	}
	var rel releaseJSON
...
func downloadFile(client *http.Client, opts Options, url, destPath string) error {
	...
	if n > MaxDownloadBytes {
		_ = os.Remove(destPath)
		return fmt.Errorf("download exceeds max size (%d bytes)", MaxDownloadBytes)
	}
	return nil
}
...
func verifyChecksum(client *http.Client, opts Options, rel *releaseJSON, archivePath, wantName string) error {
	...
	if sumURL == "" {
		return checksumMissingErr(opts, "release has no checksums.txt asset")
	}
	...
	if wantHash == "" {
		return checksumMissingErr(opts, fmt.Sprintf("checksums.txt has no entry for %s", wantName))
	}
...
func checksumMissingErr(opts Options, reason string) error {
	if opts.AllowUnverified {
		fmt.Printf("warning: %s; continuing (--allow-unverified)\n", reason)
		return nil
	}
	return fmt.Errorf("%s (refusing to install; pass --allow-unverified to override)", reason)
}

func validSHA256Hex(h string) bool {
	if len(h) != 64 {
		return false
	}
	for i := 0; i < len(h); i++ {
...
func readChecksumFor(checksumFile, filename string) (string, error) {
	...
	for sc.Scan() {
		hash, base, ok := parseChecksumLine(sc.Text())
		if !ok {
			continue
		}
		...
		if !validSHA256Hex(hash) {
			return "", fmt.Errorf("malformed sha256 for %s in checksums.txt", wantBase)
		}
		return hash, nil
	}
...
// parseChecksumLine extracts hash and basename from a checksums.txt line.
// Accepts "HASH  name", "HASH *name", and optional "sha256:" prefix.
// ok=false for blanks, comments, and malformed rows (hash validity is separate).
func parseChecksumLine(line string) (hash, base string, ok bool) {
