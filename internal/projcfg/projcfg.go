...
type Config struct {
	...
	// flowing) but only read-only tools may shadow-execute for baseline cost;
	// side-effect tools are skipped entirely. The agent always receives a
	// redirect notice so it falls back to its built-in tools.
	ToolsEnabled bool `json:"tools_enabled"`
	// Track sets how much of each call is persisted: off | summary | full.
	Track string `json:"track"`
	// MinimalTools forces the focused advertise profile for this project when
	// true. The global default is already focused (~12 entry points); set
	// CODEHELPER_TOOL_PROFILE=full (or CODEHELPER_MINIMAL_TOOLS=off) for the
	// full catalog. Hidden tools stay fully callable by name.
