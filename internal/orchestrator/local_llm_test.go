...
// deterministicChat pins tool-chain assertions to the rule-based planner. A nil
// Options.Chat is not enough: New falls back to ResolveLocalChat, so on a machine
// with a local model configured the plan (and the chosen tools) come from the LLM
// and the assertion outcome varies per developer box. A chat that always fails
// makes ClassifyTaskHybrid fall back to ClassifyTask everywhere.
func deterministicChat() LocalChat {
	return fakeChat{err: errors.New("local llm disabled in tests")}
}
