...
func ClassifyTask(task string, constraints Constraints, memoryRules []string) Plan {
	...
	// Preserve original token case so CamelCase symbols survive for exact-hit
	// ranking and investigate short-circuit (lt is only for intent scoring).
	entities := extractEntities(strings.TrimSpace(task), constraints.PreferredEntities)
