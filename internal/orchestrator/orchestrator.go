...
func (o *Orchestrator) Run(ctx context.Context, task string, constraints Constraints) (*Result, error) {
	...
	shortCircuitSym := ""
	if sym, ok := shouldShortCircuitInvestigate(plan, task); ok {
		shortCircuitSym = sym
		steps = []workflowStep{
			{Tool: "investigate", Why: "Task names an exact indexed symbol — investigate instead of vague explain", Args: map[string]any{
				"target": sym, "query": sym,
			}},
		}
	}
	pack := ContextPack{}
	...
	if shortCircuitSym != "" {
		topSymbol = shortCircuitSym
	}
	var queryText string
	if len(plan.Queries) > 0 {
		queryText = plan.Queries[0]
	}
	if bq := bestEntityQuery(plan.Entities); bq != "" {
		// Prefer CamelCase entity as the locate query even when LLM queries are verbose.
		if queryText == "" || strings.Count(queryText, " ") > 3 || looksLikeSymbolName(bq) {
			queryText = bq
		}
	}
	...
	for _, step := range steps {
		...
		case "investigate":
			if _, ok := args["target"]; !ok {
				...
				} else if bq := bestEntityQuery(plan.Entities); bq != "" {
					args["target"] = bq
				}
			}
			if _, ok := args["query"]; !ok {
				if q, _ := args["target"].(string); q != "" {
					args["query"] = q
				} else {
					args["query"] = task
				}
		...
		summary, sym, symbols, files, risks, verify := summarizeToolOutput(step.Tool, raw, err, task, plan.Entities)
		if sym != "" {
			topSymbol = sym
		}
