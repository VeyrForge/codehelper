...
func (r *Result) AgentPayload(full bool) any {
	...
	out := SlimAgentPayload{
		...
		RecommendedNextTools: orchestrateRecommendedTools(string(r.Intent), string(r.Workflow), r.ToolTraceCompact, r.ContextPack.Symbols),
...
func orchestrateRecommendedTools(intent, workflow string, trace []CompactTrace, symbols []string) []string {
	// Exact-symbol short-circuit: investigate already ran — steer to change_kit.
	if len(trace) == 1 && trace[0].Tool == "investigate" {
		return []string{"change_kit", "context", "impact", "verify"}
	}
	for _, s := range symbols {
		if isStrongSymbolToken(s) || strings.HasPrefix(s, "sym:") {
			return []string{"investigate", "change_kit", "context", "impact"}
		}
	}
