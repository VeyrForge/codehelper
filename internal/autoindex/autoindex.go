...
func (e *Engine) Run(ctx context.Context) error {
	...
	for {
		...
		case <-cooldown.C:
			...
			// Long-lived daemons miss parser/schema bumps (no file events). When
			// this process is a newer binary than meta records, force a reindex.
			if e.needsVersionReindex() {
				slog.Info("auto-index run", "reason", "parser_version", "root", e.indexRoot)
				if err := e.runIndex(ctx, "parser_version"); err != nil {
					slog.Warn("auto-index parser_version run failed", "err", err)
				} else {
					e.upsertRegistry(ctx)
				}
			}
...
func (e *Engine) needsVersionReindex() bool {
	m, err := meta.Read(e.indexRoot)
	if err != nil || m == nil {
		return false
	}
	return m.SchemaVersion != meta.SchemaVersion || m.ParserVersion != parser.Version
}
