...
func TestNeedsVersionReindex(t *testing.T) {
	dir := t.TempDir()
	e := &Engine{indexRoot: dir}
	if e.needsVersionReindex() {
		t.Fatal("missing meta should not force version reindex")
	}
	...
	if !e.needsVersionReindex() {
		t.Fatal("older parser_version should need reindex")
	}
	if err := meta.Write(dir, &meta.Data{RepoName: "x", ParserVersion: parser.Version}); err != nil {
		t.Fatal(err)
	}
	if e.needsVersionReindex() {
		t.Fatal("current parser_version should not need reindex")
	}
}
