...
func generateProfile(repoRoot string, scanSub bool) (ProjectProfile, error) {
	repoRoot = paths.ResolveWalkRoot(filepath.Clean(repoRoot))
	...
	// Deno layout (deno.json / deno.jsonc) — wins over incidental package.json
	// for project_type when no stronger framework is set later.
	if hasFile("deno.json") || hasFile("deno.jsonc") {
		pms["deno"] = struct{}{}
		langs["typescript"] = struct{}{}
		if p.ProjectType == "" || p.ProjectType == "node" {
			p.ProjectType = "deno"
		}
		if hasFile("deno.json") {
			entries = append(entries, "deno.json")
		} else {
			entries = append(entries, "deno.jsonc")
		}
		p.TestCommands = append(p.TestCommands, "deno test")
		p.LintCommands = append(p.LintCommands, "deno lint")
	}
	// Bun lock markers — package manager hint; framework refined in detectFramework.
	if hasFile("bun.lockb") || hasFile("bun.lock") || hasFile("bunfig.toml") {
		pms["bun"] = struct{}{}
		langs["typescript"] = struct{}{}
		if hasFile("bun.lockb") {
			entries = append(entries, "bun.lockb")
		} else if hasFile("bun.lock") {
			entries = append(entries, "bun.lock")
		}
		if hasFile("bunfig.toml") {
			entries = append(entries, "bunfig.toml")
		}
		if p.ProjectType == "" || p.ProjectType == "node" {
			p.ProjectType = "bun"
			p.TestCommands = append([]string{"bun test"}, p.TestCommands...)
			p.LintCommands = append([]string{"bun run lint"}, p.LintCommands...)
		}
	}
	if hasFile("wrangler.toml") || hasFile("wrangler.json") || hasFile("wrangler.jsonc") {
		langs["typescript"] = struct{}{}
		if hasFile("wrangler.toml") {
			entries = append(entries, "wrangler.toml")
		}
		if p.ProjectType == "" || p.ProjectType == "node" {
			p.ProjectType = "cloudflare_workers"
		}
	}
...
func isJSFamilyProjectType(pt string) bool {
	...
	case "node", "javascript", "typescript", "nextjs", "nuxt", "sveltekit", "remix",
		"react", "vue", "angular", "nestjs", "express", "fastify", "hono", "deno", "bun", "cloudflare_workers",
		"electron":
		return true
	default:
		return false
...
func collectLangStats(repoRoot string, langs map[string]struct{}) ([]LanguageStat, string) {
	...
	_ = filepath.WalkDir(repoRoot, func(path string, d fs.DirEntry, err error) error {
		...
		if d.IsDir() {
			...
			// gitignore entry like "/codehelper" for the built binary, or a
			// common build-dir name like "target"/"build"/"dist").
			if rel == "." {
				return nil
			}
			if ignored[base] {
