...
func TestGenerateTypeAndVersion(t *testing.T) {
	...
	t.Run("deno.json detects deno project", func(t *testing.T) {
		dir := t.TempDir()
		writeFile(t, dir, "deno.json", `{"tasks":{"start":"deno run main.ts"}}`)
		writeFile(t, dir, "main.ts", "Deno.serve(() => new Response('ok'));\n")
		p, _ := Generate(dir)
		if p.ProjectType != "deno" || p.Framework != "deno" {
			t.Errorf("got type=%q framework=%q, want deno/deno", p.ProjectType, p.Framework)
		}
		joined := strings.Join(p.PackageManagers, ",")
		if !strings.Contains(joined, "deno") {
			t.Errorf("package_managers=%v want deno", p.PackageManagers)
		}
	})

	t.Run("bun.lockb detects bun when no stronger framework", func(t *testing.T) {
		dir := t.TempDir()
		writeFile(t, dir, "package.json", `{"name":"bun-app","scripts":{"test":"bun test"}}`)
		writeFile(t, dir, "bun.lockb", "bun-lock-stub\n")
		p, _ := Generate(dir)
		if p.ProjectType != "bun" || p.Framework != "bun" {
			t.Errorf("got type=%q framework=%q, want bun/bun", p.ProjectType, p.Framework)
		}
	})

	t.Run("next.js wins over bun.lockb", func(t *testing.T) {
		dir := t.TempDir()
		writeFile(t, dir, "package.json", `{"dependencies":{"next":"^14.1.0","react":"^18.2.0"}}`)
		writeFile(t, dir, "bun.lockb", "bun-lock-stub\n")
		p, _ := Generate(dir)
		if p.ProjectType != "nextjs" || p.Framework != "nextjs" {
			t.Errorf("got type=%q framework=%q, want nextjs/nextjs", p.ProjectType, p.Framework)
		}
	})

	t.Run("wrangler.toml detects cloudflare workers", func(t *testing.T) {
		dir := t.TempDir()
		writeFile(t, dir, "wrangler.toml", "name = \"probe\"\nmain = \"src/index.ts\"\n")
		writeFile(t, dir, "src/index.ts", "export default { async fetch(){ return new Response('ok') } }\n")
		p, _ := Generate(dir)
		if p.ProjectType != "cloudflare_workers" || p.Framework != "cloudflare_workers" {
			t.Errorf("got type=%q framework=%q, want cloudflare_workers", p.ProjectType, p.Framework)
		}
	})
