...
func detectFramework(repoRoot string, p *ProjectProfile, deps []Dependency) {
	...
	// --- Deno / Bun / Cloudflare Workers (layout markers; win over bare node) ---
	if has("deno.json") || has("deno.jsonc") {
		set("deno", "deno", "")
		p.DangerZones = appendUniq(p.DangerZones, "permissions (--allow-*)", "jsr:/npm: URL imports")
		return
	}
	if has("wrangler.toml") || has("wrangler.json") || has("wrangler.jsonc") {
		// Prefer Workers when wrangler is present and no stronger Node framework yet.
		// Next/Nuxt etc. still win when declared in package.json below.
		if !has("package.json") {
			set("cloudflare_workers", "cloudflare_workers", "")
			p.DangerZones = appendUniq(p.DangerZones, "edge runtime limits", "Workers bindings")
			return
		}
	}
	...
	if has("package.json") {
		...
		case depVer["npm:electron"] != "":
			set("electron", "electron", depVer["npm:electron"])
		case isVue:
		...
		case depVer["npm:hono"] != "":
			set("hono", "hono", depVer["npm:hono"])
		case depVer["npm:fastify"] != "":
			set("fastify", "fastify", depVer["npm:fastify"])
		case depVer["npm:express"] != "":
			set("express", "", depVer["npm:express"]) // keep node type; express is a lib
		case has("wrangler.toml") || has("wrangler.json") || has("wrangler.jsonc") ||
			depVer["npm:@cloudflare/workers-types"] != "" || depVer["npm:wrangler"] != "":
			set("cloudflare_workers", "cloudflare_workers", firstNonEmpty(depVer["npm:wrangler"], depVer["npm:@cloudflare/workers-types"]))
		case has("bun.lockb") || has("bun.lock") || has("bunfig.toml"):
			set("bun", "bun", "")
		}
	} else if has("bun.lockb") || has("bun.lock") || has("bunfig.toml") {
		set("bun", "bun", "")
	}
