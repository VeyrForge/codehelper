...
func New(ctx context.Context, opt Options) (*Watcher, error) {
	...
	// Follow Windows junctions before recursive registration — WalkDir on a
	// junction root Lstats ModeIrregular and would register nothing.
	opt.Root = paths.ResolveWalkRoot(abs)
	if opt.Debounce <= 0 {
