...
// Without ResolveWalkRoot, addRecursive on a junction root registers no watches.
func TestWatcher_WindowsJunctionSeesWrites(t *testing.T) {
	...
	w, err := New(ctx, Options{
		...
		MinDebounce: 80 * time.Millisecond,
	...
	case paths := <-got:
		...
		if !found {
			t.Fatalf("expected main.go in batch, got %v", paths)
		}
	case <-time.After(2 * time.Second):
		t.Fatal("no watch event through junction root")
	}
}
