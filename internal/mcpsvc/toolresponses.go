...
func ambiguityGuard(hits []retrieval.RankedSymbol) string {
	...
	a, b := hits[0], hits[1]
	// Same bare name under different paths (common on PHP/Ruby hubs) — path= is required.
	if strings.EqualFold(a.Symbol.Name, b.Symbol.Name) {
		pa, pb := a.Symbol.Path, b.Symbol.Path
		if pa != "" && pb != "" && pa != pb {
			return fmt.Sprintf("ambiguous: %q appears in multiple paths (%s vs %s) — pass path= (or ParentID / Type.Method / sym: id) on context/impact",
				a.Symbol.Name, shortAmbiguityPath(pa), shortAmbiguityPath(pb))
		}
		return "" // same path / missing paths — silent
	}
	top, second := a.Score, b.Score
	if top <= 0 {
		return ""
	}
	if (top-second)/top < 0.08 {
...
func shortAmbiguityPath(p string) string {
...
type projectContextMCPResponse struct {
	...
	Warnings                []string               `json:"warnings,omitempty"`
	Confidence              float64                `json:"confidence,omitempty"`
	WorkspaceGroups         []workspaceGroupBrief  `json:"workspace_groups,omitempty"`
}

// workspaceGroupBrief is the agent-facing multi-root hint on project_context.
type workspaceGroupBrief struct {
	ID        string   `json:"id"`
...
func compactProjectContext(out projectContextMCPResponse) projectContextMCPResponse {
	return projectContextMCPResponse{
		...
		WorkspaceGroups:         out.WorkspaceGroups,
	}
}
...
// appendFreshnessWarnings merges warning + stale-index into Warnings once.
// Prefer the longer "index may be stale: … — fix" form when both would fire.
func appendFreshnessWarnings(warnings []string, warning string, fresh freshness.Report) []string {
	if warning != "" {
	...
	if warning != "" && strings.Contains(warning, fresh.StaleReason) {
		return warnings
	}
	return append(warnings, "stale index: "+fresh.StaleReason)
}
...
func enrichQueryToolResponse(out *queryToolResponse, hits []retrieval.RankedSymbol) {
	...
	out.Warnings = appendFreshnessWarnings(out.Warnings, out.Warning, out.Freshness)
...
func enrichImpactResponse(out *impactMCPResponse, res *types.ImpactResult) {
	...
	if res != nil {
		stampImpactProvenance(res.Nodes)
		stampImpactProvenance(res.MustUpdateCandidates)
	...
	out.Warnings = appendFreshnessWarnings(out.Warnings, out.Warning, out.Freshness)
	emptyFanout := res != nil && (strings.EqualFold(res.RiskTier, "low") || len(res.Nodes) <= 1)
	primaryLang := strings.TrimSpace(out.PrimaryLanguage)
	if out.CallGraphConfidence != "" {
		out.Warnings = appendSparseIsolationWarnings(out.Warnings, out.CallGraphConfidence, primaryLang, emptyFanout)
		if isMediumCallGraphConfidence(out.CallGraphConfidence) {
			out.Confidence = 0.65
			out.WhatNext = applyMediumWhatNextCaution(out.WhatNext, out.CallGraphConfidence)
			if out.Note == "" {
				out.Note = "Thinner call graph (MEDIUM) — callers/dependents may be incomplete; prefer path= / Type.Method; empty fanout ≠ isolation."
			}
			if emptyFanout && res != nil && strings.EqualFold(res.RiskTier, "low") {
				res.RiskTier = "unknown"
				out.Impact = res
			}
		} else {
			out.Confidence = 0.45
			out.WhatNext = applySparseWhatNextCaution(out.WhatNext, out.CallGraphConfidence)
			if res != nil && emptyFanout {
				// Never leave risk_tier=low on a LOW-confidence / self-only sparse graph.
				res.RiskTier = "unknown"
				out.Impact = res
	...
	} else if res != nil && emptyFanout && strings.EqualFold(res.RiskTier, "low") {
		// Fail closed even when density cleared the LOW label: self-only + risk_tier=low
		// must not read as isolation proof (0 callers ≠ safe).
		res.RiskTier = "unknown"
		out.Impact = res
		out.Confidence = 0.55
		out.Warnings = append(out.Warnings,
			"AGENT DIRECTIVE: risk_tier=low with 0 callers / self-only blast_radius is NOT isolation proof — confirm with tests + textual search before editing.")
		if out.Note == "" {
			out.Note = "SELF-ONLY blast_radius — do NOT treat risk_tier=low / 0 dependents as safe-to-edit isolation."
		}
		out.WhatNext = applySparseWhatNextCaution(out.WhatNext, "LOW — self-only blast_radius")
	} else {
		out.Confidence = 0.8
...
// sparseIsolationDirective is the agent-facing fail-closed copy when call graphs
// are sparse (Swift→Lua band, C++/Godot, PHP/Ruby/Java, …). Empty fanout ≠ safe edit.
const sparseIsolationDirective = "AGENT DIRECTIVE: call_graph_confidence=LOW — empty fanout / 0 callers / risk_tier=low is NOT safe isolation on sparse stacks (Swift→Lua band, C/C++/Godot, PHP/Ruby/Java). Confirm with tests + textual search before editing."

const mediumIsolationDirective = "AGENT DIRECTIVE: call_graph_confidence=MEDIUM — blast_radius may under-count DI/facade/macro callers. Prefer path= / Type.Method; empty fanout still ≠ isolation."

func appendSparseIsolationWarnings(dst []string, graphConf, primaryLang string, emptyFanout bool) []string {
	if isMediumCallGraphConfidence(graphConf) {
		dst = append(dst, mediumIsolationDirective)
	} else {
		dst = append(dst, sparseIsolationDirective)
	}
	if strings.TrimSpace(graphConf) != "" {
		dst = append(dst, "sparse call graph: risk_tier/dependents/blast_radius under-counted — NEVER treat empty blast_radius or 0 callers as isolation proof; confirm with tests + textual search")
	}
	lang := strings.ToLower(strings.TrimSpace(primaryLang))
	if lang != "" && isDynamicSparseLanguage(lang) && emptyFanout {
		dst = append(dst, fmt.Sprintf(
			"primary_language=%q routinely under-resolves calls — treat self-only / empty blast_radius as ABSTAIN, not isolation",
			lang,
		))
	}
	return dst
...
func stampImpactProvenance(nodes []types.ImpactNode) {
	for i := range nodes {
		if nodes[i].Confidence > 0 && nodes[i].Provenance == "" {
			nodes[i].Provenance = string(graph.ProvenanceFromConfidence(nodes[i].Confidence))
		}
	}
}
