...
func TestResolveKickoffTask_QueryAlias(t *testing.T) {
	...
	if !strings.Contains(note, "canonical key is task=") {
		t.Fatalf("expected param_correction-style note, got %q", note)
	}
...
func TestRelativePathRejectsSymlinkEscape(t *testing.T) {
	dir := t.TempDir()
	outside := t.TempDir()
	...
	// Existing file outside via symlink
	secret := filepath.Join(outside, "secret.txt")
	if err := os.WriteFile(secret, []byte("x"), 0o600); err != nil {
		t.Fatal(err)
	}
	if _, err := relativePathUnderRepo(dir, "external/secret.txt"); err == nil {
		t.Fatal("expected symlink escape rejection for existing file")
	}
	// New file under symlink parent must also be rejected (nearest-parent resolve)
	if _, err := relativePathUnderRepo(dir, "external/new-file.txt"); err == nil {
		t.Fatal("expected symlink escape rejection for new file under escaped parent")
	}
	// Normal in-repo path still works
	if err := os.MkdirAll(filepath.Join(dir, "ok"), 0o755); err != nil {
		t.Fatal(err)
	}
	rel, err := relativePathUnderRepo(dir, "ok/a.txt")
	if err != nil || rel != "ok/a.txt" {
		t.Fatalf("in-repo path: rel=%q err=%v", rel, err)
	}
}

// symlinkOrSkip creates a symlink, skipping the test where the platform or the
// current account cannot create one (Windows without Developer Mode or admin).
...
func TestRelativePathRejectsSymlinkedFileEscape(t *testing.T) {
	dir := t.TempDir()
	outside := t.TempDir()
	...
	symlinkOrSkip(t, secret, filepath.Join(dir, "key.txt"))
	if _, err := relativePathUnderRepo(dir, "key.txt"); err == nil {
		t.Fatal("expected a symlinked file pointing outside the repo to be rejected")
	}
}

func TestRelativePathRejectsSymlinkEscapeVariants(t *testing.T) {
	dir := t.TempDir()
	outside := t.TempDir()
	symlinkOrSkip(t, outside, filepath.Join(dir, "external"))

	cases := map[string]string{
		"the escaping symlink itself":       "external",
		"nested path that does not exist":   "external/a/b/c.txt",
		"traversal folded back into a link": "external/../external/secret.txt",
		"absolute path through the link":    filepath.Join(dir, "external", "secret.txt"),
	}
	for name, p := range cases {
		if _, err := relativePathUnderRepo(dir, p); err == nil {
			t.Errorf("%s: expected %q to be rejected", name, p)
		}
	}
}

// A repo whose root is itself reached through a symlink must keep working — the
// containment check resolves both sides, so it must not reject its own files.
func TestRelativePathAllowsSymlinkedRepoRoot(t *testing.T) {
	base := t.TempDir()
	realRoot := filepath.Join(base, "real")
	if err := os.MkdirAll(filepath.Join(realRoot, "sub"), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(realRoot, "sub", "f.go"), []byte("package p\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	link := filepath.Join(base, "link")
	symlinkOrSkip(t, realRoot, link)

	cases := map[string]string{
		"relative path":              "sub/f.go",
		"absolute real path":         filepath.Join(realRoot, "sub", "f.go"),
		"absolute path via the link": filepath.Join(link, "sub", "f.go"),
	}
	for name, p := range cases {
		rel, err := relativePathUnderRepo(link, p)
		if err != nil || rel != "sub/f.go" {
			t.Errorf("%s: rel=%q err=%v, want sub/f.go", name, rel, err)
		}
	}
	if rel, err := relativePathUnderRepo(link, "sub/new.go"); err != nil || rel != "sub/new.go" {
		t.Errorf("new file under a symlinked root: rel=%q err=%v", rel, err)
	}
}

// A symlink that stays inside the repo is allowed, but resolves to its real
// location — otherwise an alias would be a way around workspacePathBlocked.
func TestRelativePathNormalizesInRepoSymlinkAlias(t *testing.T) {
	dir := t.TempDir()
	...
	symlinkOrSkip(t, gitDir, filepath.Join(dir, "alias"))

	rel, err := relativePathUnderRepo(dir, "alias/config")
	if err != nil {
		t.Fatalf("an in-repo alias should resolve, not error: %v", err)
	}
	if rel != ".git/config" {
		t.Fatalf("alias should normalize to its real location, got %q", rel)
	}
	if !workspacePathBlocked(rel, false) {
		t.Fatal("the resolved .git path must still be blocked")
	}
}

func TestAbsPathUnderRepoRejectsSymlinkEscape(t *testing.T) {
	dir := t.TempDir()
	outside := t.TempDir()
	symlinkOrSkip(t, outside, filepath.Join(dir, "external"))
	secret := filepath.Join(outside, "secret.txt")
	...
	if _, err := absPathUnderRepo(dir, "external/secret.txt"); err == nil {
		t.Fatal("expected absPathUnderRepo to reject symlink escape")
	}
	// In-repo file still resolves to an absolute path under the repo.
	if err := os.MkdirAll(filepath.Join(dir, "ok"), 0o755); err != nil {
		t.Fatal(err)
	}
	want := filepath.Join(dir, "ok", "a.txt")
	if err := os.WriteFile(want, []byte("x"), 0o644); err != nil {
		t.Fatal(err)
	}
	got, err := absPathUnderRepo(dir, "ok/a.txt")
	if err != nil {
		t.Fatal(err)
	}
	...
	if gotResolved != wantResolved {
		t.Fatalf("absPathUnderRepo=%q want %q", gotResolved, wantResolved)
	}
}

func TestRevalidateAbsUnderRepoDetectsSymlinkSwap(t *testing.T) {
	dir := t.TempDir()
	outside := t.TempDir()
	parent := filepath.Join(dir, "ok")
	if err := os.MkdirAll(parent, 0o755); err != nil {
		t.Fatal(err)
	}
	abs := filepath.Join(parent, "a.txt")

	safe, err := revalidateAbsUnderRepo(dir, abs)
	if err != nil {
		t.Fatalf("initial revalidate should pass for in-repo path: %v", err)
	}
	if safe == "" {
		t.Fatal("expected non-empty safe path")
	}

	// Simulate TOCTOU: after validation, replace the parent with an escaping symlink.
	if err := os.RemoveAll(parent); err != nil {
		t.Fatal(err)
	}
	symlinkOrSkip(t, outside, parent)

	if _, err := revalidateAbsUnderRepo(dir, abs); err == nil {
		t.Fatal("expected revalidateAbsUnderRepo to reject path after symlink swap")
	}
	if err := atomicWriteUnderRepo(dir, abs, []byte("pwned")); err == nil {
		t.Fatal("expected atomicWriteUnderRepo to refuse write after symlink swap")
	}
	...
	if len(entries) != 0 {
		t.Fatalf("write escaped into outside dir: %v", entries)
	}
}

func TestRevalidateAbsUnderRepoAllowsInRepo(t *testing.T) {
	dir := t.TempDir()
	if err := os.MkdirAll(filepath.Join(dir, "ok"), 0o755); err != nil {
		t.Fatal(err)
	}
	abs := filepath.Join(dir, "ok", "a.txt")
	if err := os.WriteFile(abs, []byte("x"), 0o644); err != nil {
		t.Fatal(err)
	}
	got, err := revalidateAbsUnderRepo(dir, abs)
	if err != nil {
		t.Fatal(err)
	}
	gotResolved, _ := filepath.EvalSymlinks(got)
	wantResolved, _ := filepath.EvalSymlinks(abs)
	if gotResolved != wantResolved {
		t.Fatalf("revalidateAbsUnderRepo=%q want %q", gotResolved, wantResolved)
	}
	if err := atomicWriteUnderRepo(dir, abs, []byte("y")); err != nil {
		t.Fatal(err)
	}
	data, err := os.ReadFile(abs)
	if err != nil || string(data) != "y" {
		t.Fatalf("in-repo write failed: data=%q err=%v", data, err)
	}
}

func TestReadSymbolDefinitionRejectsSymlinkEscape(t *testing.T) {
	dir := t.TempDir()
	outside := t.TempDir()
	symlinkOrSkip(t, outside, filepath.Join(dir, "external"))
	...
	sym := types.Symbol{Path: "external/secret.go", LineStart: 1, LineEnd: 2}
	if got := readSymbolDefinition(dir, sym, 40); got != "" {
		t.Fatalf("readSymbolDefinition must not follow escaping symlink, got %q", got)
	}
}
