...
func TestPythonDiagCmds_FallsBackToCompileall(t *testing.T) {
	dir := t.TempDir()
	// Preference is pyright → mypy → ruff → compileall; skip when a stronger
	// checker is on PATH (common on Windows developer machines).
	for _, bin := range []string{"pyright", "mypy", "ruff"} {
		if _, err := exec.LookPath(bin); err == nil {
			t.Skipf("%s on PATH; compileall fallback not selected", bin)
		}
	}
