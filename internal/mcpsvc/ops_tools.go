...
func RegisterOpsTools(s *server.MCPServer, reg *registry.Registry) {
	...
	s.AddTool(mcp.NewTool("env_context",
		mcp.WithDescription("Read-only project environment snapshot: detected toolchain versions, package scripts (npm/make), docker-compose hint, and configured aliases/log sources from files — one call, no shell. Use before verify/run_alias to see available scripts; does not run commands or edit files."),
		mcp.WithString("repo", mcp.Description("Repository name")),
		mcp.WithString("format", mcp.Description("toon (default) | json")),
		annotReadOnlyClosedWorld(),
