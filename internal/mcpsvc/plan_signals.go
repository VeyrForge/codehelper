...
type taskSignals struct {
	...
	SparseGraph bool            // call_graph_confidence=LOW — never imply isolation
}
	...
	if t := sig.Top; t != nil {
		...
		if sig.SparseGraph {
			...
			dp = append(dp, fmt.Sprintf(
				"Call graph is SPARSE (confidence=LOW) for `%s` — do NOT treat %d dependents / empty fanout / risk_tier=low as safe-to-edit isolation; confirm with tests + textual search before editing.",
				t.Name, sig.Dependents))
		}
		dp = append(dp, fmt.Sprintf(
...
// Paths are slash-normalized so Windows backslashes do not leak into callers.
...
func distinctPkgs(nodes []types.ImpactNode) int {
	...
	for _, n := range nodes {
		...
		seen[filepath.ToSlash(filepath.Dir(p))] = struct{}{}
	}
	return len(seen)
}
