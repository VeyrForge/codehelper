...
func lspHandler(reg *registry.Registry) server.ToolHandlerFunc {
	return func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		res, err := lsp.Query(ctx, repo.RootPath, lsp.Position{Path: path, Line: line, Col: col}, lsp.Action(action))
		if err != nil {
			return mcp.NewToolResultError(err.Error()), nil
		}
		out := lspResponse{
		...
		if st, gerr := openGraph(repo.RootPath); gerr == nil {
			...
			if res.Fallback || !res.OK {
				if out.Note == "" {
					out.Note = "LSP unavailable or empty — use graph_symbols / context / query / trace."
				} else if !strings.Contains(out.Note, "graph") {
					out.Note += " Prefer graph_symbols / context / query when Fallback is true."
				}
				out.WhatNext = "Call context name=<symbol> or query — do not retry lsp with the same empty path/line"
				out.RecommendedNextTools = []string{"context", "query", "impact", "trace"}
			}
		}
		if out.Note == "" && out.OK {
			out.Note = "LSP result merged with graph_symbols when indexed. Default navigation remains query/context/trace."
		}
...
// RegisterLSPTools registers the optional LSP bridge (hover/definition/references).
func RegisterLSPTools(s *server.MCPServer, reg *registry.Registry) {
	s.AddTool(mcp.NewTool("lsp",
		mcp.WithDescription("Optional LSP bridge (Serena-parity, pragmatic): hover / definition / references via gopls, typescript-language-server, or pyright when installed. Merges graph_symbols IDs with LSP results. Graceful fallback to graph when binaries missing or CODEHELPER_LSP=0. Does NOT replace query/context/trace — use those by default; lsp when you need compiler-grade types at a file:line."),
		mcp.WithString("action", mcp.Required(), mcp.Description("hover | definition (alias def) | references (alias refs)")),
