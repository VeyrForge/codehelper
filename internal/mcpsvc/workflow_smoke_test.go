...
func smokeWorkflowBed(t *testing.T, handlers map[string]server.ToolHandlerFunc, root, expectRepo string) map[string]any {
	...
	checks := []check{
		...
		{"query_core", "query", map[string]any{"q": workflowQueryFor(root, expectRepo)}},
		{"context_core", "context", map[string]any{"name": workflowContextName(root, expectRepo)}},
		{"impact_core", "impact", map[string]any{"name": workflowImpactName(root, expectRepo)}},
		{"kickoff", "kickoff", map[string]any{"task": workflowTaskFor(root, expectRepo)}},
	}
...
func nestWorkflowGold(root, repo string) string {
	if repo == "nest-starter" {
		return "CatsService"
	}
	if repo == "nest" {
...
func workflowQueryFor(root, repo string) string {
	if g := nestWorkflowGold(root, repo); g != "" {
		return g
	}
	switch repo {
	case "express":
...
func workflowContextName(root, repo string) string {
	if g := nestWorkflowGold(root, repo); g != "" {
		return g
	}
	switch repo {
...
func workflowImpactName(root, repo string) string {
	return workflowContextName(root, repo)
}

func workflowTaskFor(root, repo string) string {
	if g := nestWorkflowGold(root, repo); g != "" {
		return "What depends on " + g + " before changing it?"
	}
	switch repo {
	case "express":
