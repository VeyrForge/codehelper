...
func hitPath(name, path string, score float64) retrieval.RankedSymbol {
	return retrieval.RankedSymbol{Symbol: types.Symbol{Name: name, Path: path}, Score: score}
}

// TestAmbiguityGuard: warn only when the top two are near-tied; stay silent (no
// token cost) for a clear winner, a single hit, or duplicate-named hits without paths.
func TestAmbiguityGuard(t *testing.T) {
	...
	// duplicate names without paths -> silent (legacy)
	if g := ambiguityGuard([]retrieval.RankedSymbol{hit("model", 1.0), hit("model", 0.99)}); g != "" {
		t.Errorf("duplicate-named hits without paths should be silent, got %q", g)
	}
	// same name, different paths -> path= guidance (PHP/Ruby hubs)
	g := ambiguityGuard([]retrieval.RankedSymbol{
		hitPath("User", "app/Models/User.php", 1.0),
		hitPath("User", "database/factories/UserFactory.php", 0.95),
	})
	if !strings.Contains(g, "path=") || !strings.Contains(g, "User") {
		t.Errorf("same-name different paths should suggest path=, got %q", g)
	}
}
