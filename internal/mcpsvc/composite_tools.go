...
func RegisterCompositeTools(s *server.MCPServer, reg *registry.Registry) {
	...
	s.AddTool(mcp.NewTool("investigate",
		mcp.WithDescription("UNDERSTAND composite (PREFERRED over chaining query→context→impact) — by default query + context + impact + test_impact in one call. Pass recipe=architecture|dead_code|security|perf for specialized audits. Returns a compact TOON bundle by default (format=json for JSON). Prefer kickoff/orchestrate for open-ended feature starts; use investigate when you already know the locate question."),
...
func investigateHandler(reg *registry.Registry, h map[string]server.ToolHandlerFunc) server.ToolHandlerFunc {
	return func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if target == "" {
			activeRepo := repoName
			if e, err := resolveRepo(ctx, reg, repoName); err == nil {
				activeRepo = e.Name
			}
			return mustToolResultFormatted(scrubHostBleedPayload(map[string]any{
		...
		return mustToolResultFormatted(scrubHostBleedPayload(out, activeRepo), format)
	}
}
