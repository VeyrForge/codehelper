...
func vibeNextQueries(role string, shape security.ProjectShape, abstain bool, task, repoRoot string) []string {
	...
	case "performance":
		if shape == security.ShapeLibrary || shape == security.ShapeFrameworkCore {
			return performanceFrameworkNextQueries(eco)
		}
		if shape == security.ShapeSkeleton {
			return []string{
				"hotspots — expect thin/convention paths; do not invent production N+1 yet",
				"query: pagination eager load with select_related includes Model::all foreach",
				"Add a realistic list endpoint, then re-run `plan role=performance` / `hotspots`",
			}
		}
		return []string{
	...
	default:
		if isEndpointFeatureTask(task) {
			if abstain {
				...
				case "c":
					return []string{
						"context name=pingCommand — Redis PING is the liveness equivalent (not HTTP /health)",
						"query: pingCommand PONG processCommand dispatch",
						"Ask user: is HTTP /health needed, or does redis-cli PING already satisfy?",
					}
				case "vue", "svelte":
					uiHint := "createApp mount compile"
					if eco == "svelte" {
						uiHint = "mount compile component"
					}
					return []string{
						"query: " + uiHint + " — confirm UI compiler/runtime (no HTTP server)",
						"scout task=\"add GET /health\" — expect ABSTAIN; do not invent /health here",
						"Ask user: which HTTP app/demo repo should get /health, or drop the ask?",
					}
				}
				return []string{
...
func detectEcosystem(repoRoot string) string {
	...
	// Windows junctions Lstat as ModeIrregular — WalkDir on src/ would see 0 .c files.
	root = paths.ResolveWalkRoot(filepath.Clean(root))
	...
	// UI compilers before generic package.json → "node".
	if fileExists(filepath.Join(root, "packages", "runtime-core", "src", "renderer.ts")) ||
		fileExists(filepath.Join(root, "packages", "compiler-core", "src", "index.ts")) ||
		base == "vue" {
		return "vue"
	}
	if fileExists(filepath.Join(root, "packages", "svelte", "src", "compiler", "index.js")) ||
		fileExists(filepath.Join(root, "packages", "svelte", "src", "compiler", "index.ts")) ||
		base == "svelte" {
		return "svelte"
	}
	...
	srcDir := paths.ResolveWalkRoot(filepath.Join(root, "src"))
	if dirExists(srcDir) {
		n := 0
...
// performanceFrameworkNextQueries gives juniors stack-specific measurable next steps
// (alloc/hot-path/SLOWLOG) — never app N+1 hunts in library cores.
func performanceFrameworkNextQueries(eco string) []string {
	switch eco {
	case "c":
		return []string{
			"hotspots — prefer src/server.c processCommand / networking readQueryFromClient",
			"query: processCommand beforeSleep lookupKey SLOWLOG KEYS SCAN",
			"context on processCommand — measure with SLOWLOG before micro-opts",
		}
	case "python":
		return []string{
			"hotspots — prefer django/db/models/query.py QuerySet / _fetch_all",
			"query: QuerySet _fetch_all iterator select_related prefetch_related",
			"context on QuerySet — at call sites use select_related/prefetch + iterator()",
		}
	case "go":
		return []string{
			"hotspots — prefer gin.go ServeHTTP / context.go Next",
			"query: ServeHTTP handleHTTPRequest Context.Next reset bind JSON",
			"context on ServeHTTP — keep middleware/binding lean; never retain *Context",
		}
	case "node":
		return []string{
			"hotspots — prefer lib/application.js app.handle / lib/response.js res.send",
			"query: app.handle middleware chain res.send Buffer stream body",
			"context on app.handle — measure sync middleware cost before micro-opts",
		}
	case "php":
		return []string{
			"hotspots — prefer routes/controllers over migrations; check commits_scanned",
			"query: pagination with() loadMissing Model::all foreach N+1",
			"Add a realistic list endpoint then re-run plan role=performance",
		}
	default:
		return []string{
...
// preferPerfFindingNextQueries prepends cite-grounded follow-ups when performance
// findings already name real file:line hot paths (juniors can paste immediately).
func preferPerfFindingNextQueries(base []string, findings []auditFinding) []string {
	if len(findings) == 0 {
		return base
	}
	f := findings[0]
	...
	out := []string{
		fmt.Sprintf("read_workspace_file %s — confirm the hot-path cite before optimizing", cite),
		fmt.Sprintf("context on symbol at %s; then hotspots", cite),
	}
	if len(base) > 0 {
		out = append(out, base[0])
	} else {
		out = append(out, "impact + test_impact before changing the hot path")
	}
	if len(out) > 3 {
		out = out[:3]
	}
...
func buildWhatNext(role string, top *reuseCandidate, findings []auditFinding, abstain string, next []string) string {
	...
	if abstain != "" && len(findings) == 0 {
		// Grounded non-HTTP equivalent (Redis pingCommand): point there, don't
		// pretend a CVE hunt failed or that inventing /health is next.
		if top != nil && top.Name != "" &&
			(strings.EqualFold(top.Kind, "equivalent") || strings.EqualFold(top.Name, "pingCommand")) {
			return fmt.Sprintf(
				"ABSTAIN HTTP /health — use existing `%s` at %s. Paste next: `%s`",
				top.Name, top.Loc, firstOr(next, "context name="+top.Name),
			)
		}
	...
	case "security":
		if len(findings) > 0 {
			...
			if securityFindingsAreGuidanceOnly(findings) {
				why := truncateRunes(strings.TrimSpace(f.Hint), 120)
				if why == "" {
					why = "config/bootstrap footgun — grounded file:line, not an exploit proof"
				}
				return fmt.Sprintf(
					"Guidance only (not a CVE): review `%s:%d` (%s) — %s. Then hunt app sinks — paste: `%s`",
					f.File, f.Line, f.Rule, why, firstOr(next, "investigate recipe=security"),
				)
			}
			label := "Inspect"
...
func enrichPerfFindingsWhy(findings []auditFinding, shape security.ProjectShape) []auditFinding {
	...
	case security.ShapeLibrary, security.ShapeFrameworkCore:
		...
		whyDefault = "Library/framework hot path — alloc/complexity here affects every consumer; measure before micro-opts."
		if findings[0].Evidence != "" && strings.TrimSpace(findings[0].Hint) == "" {
			// Evidence already carries the junior-facing measured guidance.
		}
...
// isBareHealthishTarget is a vibe typing of lowercase "health"/"ready"/paths — not a
// concrete symbol like getHealth / HealthController / Unity's Health class that should
// resolve from the graph when present. Case-sensitive: "Health" is a type name.
