...
func writeWorkspaceFileHandler(reg *registry.Registry) server.ToolHandlerFunc {
	return func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		abs, err := absPathUnderRepo(repo.RootPath, rawPath)
		if err != nil {
			return mcp.NewToolResultError(err.Error()), nil
		}
		...
		if createDirs {
			// Re-check immediately before MkdirAll so a raced symlink parent
			// cannot redirect directory creation outside the repo.
			abs, err = revalidateAbsUnderRepo(repo.RootPath, abs)
			if err != nil {
				return mcp.NewToolResultError(err.Error()), nil
			}
			...
			// MkdirAll widens the TOCTOU window; re-check before any read/write.
			abs, err = revalidateAbsUnderRepo(repo.RootPath, abs)
			if err != nil {
				return mcp.NewToolResultError(err.Error()), nil
			}
		...
		if err := atomicWriteUnderRepo(repo.RootPath, abs, []byte(content)); err != nil {
			return mcp.NewToolResultError(err.Error()), nil
		}
...
func revertWorkspaceEditHandler(reg *registry.Registry) server.ToolHandlerFunc {
	return func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		// The snapshot's rel_path comes from a file on disk, so re-validate it against
		// the repo root instead of trusting it to still point inside the workspace.
		rel, err := relativePathUnderRepo(snap.RepoRoot, snap.RelPath)
		if err != nil {
			return mcp.NewToolResultError("snapshot target is not under the repository root: " + err.Error()), nil
		}
		if workspacePathBlocked(rel, true) {
			return mcp.NewToolResultError("reverting this path is not allowed (.git, node_modules, or .env*)"), nil
		}
		abs, err := absPathUnderRepo(snap.RepoRoot, snap.RelPath)
		if err != nil {
			return mcp.NewToolResultError("snapshot target is not under the repository root: " + err.Error()), nil
		}
		if err := atomicWriteUnderRepo(snap.RepoRoot, abs, snap.Content); err != nil {
			return mcp.NewToolResultError(err.Error()), nil
		}
...
// Both the repo root and the target are resolved through EvalSymlinks so a symlink
// inside the repo cannot escape (e.g. repo/external -> /home/user). For paths that
// do not exist yet (new files), the nearest existing parent is resolved instead.
func relativePathUnderRepo(repoRoot, userPath string) (string, error) {
	rr, err := filepath.Abs(filepath.Clean(repoRoot))
	if err != nil {
		return "", err
	}
	if resolved, err := filepath.EvalSymlinks(rr); err == nil {
		rr = resolved
	}
	...
	absFile, err = evalSymlinksNearest(absFile)
	if err != nil {
		return "", err
	}
...
// absPathUnderRepo resolves userPath under repoRoot with the same EvalSymlinks
// containment checks as relativePathUnderRepo, returning an absolute filesystem
// path safe to open/stat/read. Prefer this over filepath.Join(root, indexPath)
// for any disk I/O driven by indexed or caller-supplied relative paths.
func absPathUnderRepo(repoRoot, userPath string) (string, error) {
	rel, err := relativePathUnderRepo(repoRoot, userPath)
	if err != nil {
		return "", err
	}
	rr, err := filepath.Abs(filepath.Clean(repoRoot))
	...
	return filepath.Join(rr, filepath.FromSlash(rel)), nil
}

// revalidateAbsUnderRepo re-checks that absPath still resolves under repoRoot.
// Call immediately before MkdirAll/open/write after any gap following the initial
// relativePathUnderRepo/absPathUnderRepo check (especially after MkdirAll).
//
// This narrows the TOCTOU window between path validation and disk I/O. A portable
// openat2(RESOLVE_BENEATH)/O_NOFOLLOW open is not available across Windows+Unix
// for our rename-based atomicWrite, so a last-moment EvalSymlinks containment
// check is the practical mitigation. Residual race: a symlink swap in the
// nanoseconds between this check and CreateTemp/Rename can still redirect I/O.
func revalidateAbsUnderRepo(repoRoot, absPath string) (string, error) {
	return absPathUnderRepo(repoRoot, absPath)
}

// atomicWriteUnderRepo revalidates containment then writes. Prefer this over
// bare atomicWrite for workspace-tool mutations of caller/index-supplied paths.
func atomicWriteUnderRepo(repoRoot, absPath string, data []byte) error {
	safe, err := revalidateAbsUnderRepo(repoRoot, absPath)
	if err != nil {
		return fmt.Errorf("path no longer under repository root: %w", err)
	}
	return atomicWrite(safe, data)
}

// evalSymlinksNearest resolves symlinks for path. If path does not exist, walks
// up to the nearest existing ancestor, EvalSymlinks that, then rejoins the
// missing suffix — so writes to new files cannot escape via a parent symlink.
func evalSymlinksNearest(path string) (string, error) {
	...
	// Nothing on the path existed / could be resolved — return cleaned abs path.
	return path, nil
}
