...
func changeKitHandler(reg *registry.Registry) server.ToolHandlerFunc {
	return func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		sym, cands, rerr := resolveSymbolByName(ctx, st, repo.Name, target, wantPath, wantLine, repo.RootPath)
		if rerr != nil || sym == nil {
		...
		out := changeKitResponse{Target: apiSym{
			...
			Loc:  symbolDefLoc(repo.RootPath, *sym),
		...
		if aliasNote != "" {
			out.ParamCorrection = aliasNote
			if out.Note != "" {
				out.Note = aliasNote + " | " + out.Note
			} else {
				out.Note = aliasNote
			}
		}
...
func changeKitHealthishFallback(root, target, format string) (*mcp.CallToolResult, bool) {
	...
	readDef := func(loc string) (pathOnly string, lineNo int, def string) {
		...
		abs, aerr := absPathUnderRepo(root, pathOnly)
		if aerr != nil {
			return pathOnly, lineNo, ""
		}
	...
	if surface.Capable {
		...
		// Some non-HTTP cores already have a real liveness-equivalent (Redis
		// PING) — surface it so ABSTAIN is not a dead end for the agent.
		if eqName, eqLoc, eqNote := nonHTTPHealthEquivalent(root); eqName != "" {
			msg += "Closest existing equivalent: `" + eqName + "` at `" + eqLoc + "` — " + eqNote + " "
			next = []string{
				"context name=" + eqName + " — confirm this is the actual liveness/health equivalent",
				"query: liveness check ping command dispatch",
				"Ask user: is a real HTTP /health needed, or does the existing " + eqName + " already satisfy the ask?",
			}
		}
	} else {
...
func readSymbolDefinition(root string, sym types.Symbol, maxLines int) string {
	abs, err := absPathUnderRepo(root, sym.Path)
	if err != nil {
		return ""
	}
...
type findImplResponse struct {
	...
	Source          string          `json:"source,omitempty"` // graph | graph+lsp | graph (lsp unavailable)
	Note            string          `json:"note"`
}
...
func findImplementationsHandler(reg *registry.Registry) server.ToolHandlerFunc {
	return func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		out := findImplResponse{Interface: sym.Name, Methods: methods, Source: "graph"}
		if len(methods) == 0 {
		...
		// Optional LSP textDocument/implementation merge (gopls when installed).
		if enrichImplsWithLSP(ctx, repo.RootPath, *sym, st, repo.Name, &out, seenType) {
			out.Source = "graph+lsp"
		}
		switch {
		case len(full) == 0 && len(partial) == 0 && len(out.Implementations) == 0:
			out.Note = fmt.Sprintf("no type implements %s by structural method-name match. Implementations may live in dependencies, or use embedding the static graph didn't resolve. Try lsp action=implementations when gopls is installed.", sym.Name)
		case len(full) == 0 && len(out.Implementations) == 0:
		...
		default:
			...
			if out.Source == "graph+lsp" {
				out.Note += " LSP implementation locations merged (source=graph+lsp)."
			}
		}
		return mustToolResultFormatted(out, resolveFormat(args))
	}
...
// enrichImplsWithLSP adds types from textDocument/implementation when gopls (or
// another server) is available. Returns true when at least one LSP impl was added.
func enrichImplsWithLSP(ctx context.Context, root string, sym types.Symbol, st *graph.Store, repoID string, out *findImplResponse, seenType map[string]bool) bool {
	if out == nil || !lsp.Enabled() {
		return false
	}
	col := 1
	if abs, aerr := absPathUnderRepo(root, sym.Path); aerr == nil {
		if lines, err := readFileLines(abs); err == nil {
			if sym.LineStart >= 1 && sym.LineStart <= len(lines) {
				if idx := strings.Index(lines[sym.LineStart-1], sym.Name); idx >= 0 {
					col = idx + 1
				}
			}
		}
	...
	res, err := lsp.Query(ctx, root, lsp.Position{
		Path: sym.Path, Line: sym.LineStart, Col: col,
	}, lsp.ActionImplementations)
	if err != nil || res == nil || res.Fallback || !res.OK {
		return false
	}
	added := false
	for _, loc := range res.Locations {
		syms, _ := st.SymbolsForPath(ctx, repoID, loc.Path)
		var best *types.Symbol
		for i := range syms {
			s := &syms[i]
			if s.LineStart <= loc.Line && (s.LineEnd == 0 || s.LineEnd >= loc.Line) {
		...
		name := best.Name
		if best.Kind == types.SymbolKindMethod && best.ParentID != "" {
			name = best.ParentID
		}
		if seenType[name] {
			continue
		...
		seenType[name] = true
		out.Implementations = append(out.Implementations, implCandidate{
			Type: name,
			Loc:  fmt.Sprintf("%s:%d", best.Path, best.LineStart),
		})
		added = true
	}
	return added
