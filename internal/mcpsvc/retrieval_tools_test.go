...
func TestRetrievalFacadesInCatalog(t *testing.T) {
	...
	for _, want := range []string{"search_hybrid", "context_bundle"} {
		...
		// Full-catalog retrieval facades: callable by name, not on the focused
		// advertise list (prefer query / context / investigate composites).
		if IsFocusedTool(want) {
			t.Fatalf("%s should stay off focused advertise (full profile / by-name only)", want)
		}
	}
}
