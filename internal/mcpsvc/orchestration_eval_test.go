...
// Opt-in only — touches every registry repo and can take 30m+:
//
//	CODEHELPER_ORCH_BENCH=1 go test ./internal/mcpsvc -run TestOrchestrationBenchmark -v -timeout 30m
func TestOrchestrationBenchmark(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping full cross-project benchmark in -short mode")
	}
	if os.Getenv("CODEHELPER_ORCH_BENCH") != "1" {
		t.Skip("set CODEHELPER_ORCH_BENCH=1 to run the full cross-project orchestration benchmark")
	}
