	...
	return func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		for _, s := range syms {
			...
			if prev, ok := bestRank[s.Path]; !ok || rank < prev {
				...
				bestName[s.Path] = s.Name
				bestKind[s.Path] = string(s.Kind)
			}
		}
		...
		annotateHotspotWhy(out.Hotspots, len(commits), shape, bestLine, bestName, bestKind, repo.RootPath)
...
func isHotspotNoisePath(file, primary string) bool {
	...
	if strings.Contains(lower, "/deps/") || strings.HasPrefix(lower, "deps/") ||
		...
		strings.Contains(lower, "/scripts/") || strings.HasPrefix(lower, "scripts/") ||
		strings.Contains(lower, "/script/") || strings.HasPrefix(lower, "script/") ||
		strings.Contains(lower, "/tools/") || strings.HasPrefix(lower, "tools/") ||
		strings.Contains(lower, "/tooling/") ||
		strings.Contains(lower, "/benchmarks/") || strings.HasPrefix(lower, "benchmarks/") ||
		strings.Contains(lower, "/bench/") || strings.HasPrefix(lower, "bench/") ||
		strings.Contains(lower, "/generators/") || strings.HasPrefix(lower, "generators/") ||
		...
		strings.HasSuffix(base, ".sh") || strings.HasSuffix(base, ".ps1") ||
		strings.HasSuffix(base, ".bat") || strings.HasPrefix(base, "generate_") ||
		// Schema churn ≠ request hot path (Laravel/Rails skeletons otherwise top migrations).
		strings.Contains(lower, "/migrations/") || strings.Contains(lower, "/database/migrations/") {
		return true
	}
...
func seedLibraryCoreHotspots(rows []hotspotRow, root string, shape security.ProjectShape, topK int) []hotspotRow {
	...
	case security.ShapeSkeleton:
		// Migrations/churn noise often fills thin skeletons — seed convention paths.
		guidance = security.SkeletonPerfGuidance(root, 4)
	default:
		if len(rows) == 0 {
	...
	for i, g := range guidance {
		...
		seen[key] = struct{}{}
		seeded = append(seeded, hotspotRow{
			File: g.File, Line: g.Line, Commits: 0, Centrality: 1000 - i, Score: round3(1.0 - float64(i)*0.05),
		})
	}
	if len(seeded) == 0 {
		return rows
...
// line from the graph; fall back to name-aware disk def scan / LineForHotPath;
// only then line 1.
func annotateHotspotWhy(rows []hotspotRow, commitsScanned int, shape security.ProjectShape, bestLine map[string]int, bestName, bestKind map[string]string, root string) {
	for i := range rows {
		r := &rows[i]
		...
		if r.Line <= 0 {
			if bestLine != nil {
				if ln := bestLine[key]; ln > 0 {
					...
					for k, ln := range bestLine {
						if ln > 0 && strings.ReplaceAll(k, "\\", "/") == key {
							r.Line = ln
							break
						}
					}
		...
		// Upgrade weak cites: prefer known framework hot-path needles (app.handle,
		// ServeHTTP, processCommand) over first-in-file helpers; fall back to
		// symbol-def / LineForHotPath when still ≤1.
		if root != "" {
			if abs, err := absPathUnderRepo(root, key); err == nil {
				ln := r.Line
				libPreferred := false
				if shape == security.ShapeLibrary || shape == security.ShapeFrameworkCore {
					// Pass orig=1 so PreferLibraryHotPathLine always tries needle +
					// LineForHotPath — then apply even when graph bestLine is a later
					// (higher) weak cite that would lose a naive ln > r.Line check.
					if pref := security.PreferLibraryHotPathLine(root, key, 1); pref > 1 {
						ln = pref
						libPreferred = true
					}
				}
				if ln <= 1 {
					...
					if bestName != nil {
						name = bestName[key]
						if name == "" {
							name = bestName[r.File]
						}
					}
					if bestKind != nil {
						kind = bestKind[key]
						if kind == "" {
							kind = bestKind[r.File]
						}
					}
					if name != "" {
						if def := security.LineForSymbolDef(abs, name, kind, ln); def > ln {
							ln = def
						}
					}
					if ln <= 1 {
						if hot := security.LineForHotPath(abs); hot > ln {
							ln = hot
						}
					}
				}
				if libPreferred || ln > r.Line {
					r.Line = ln
				}
			}
		}
...
func suggestHotspotRewrite(file string, shape security.ProjectShape) string {
	...
	case security.ShapeLibrary, security.ShapeFrameworkCore:
		switch {
		case strings.HasSuffix(f, "/application.js") || strings.Contains(f, "/router/"):
			return "Profile middleware chain length + sync work in app.handle; stream large bodies — keep public API identical."
		case strings.HasSuffix(f, "/response.js"):
			return "Avoid huge sync res.send/json copies; stream or chunk large payloads under load."
		case strings.HasSuffix(f, "gin.go") || strings.HasSuffix(f, "/context.go"):
			return "Keep middleware + binding lean on ServeHTTP/Next; never retain *gin.Context past the request."
		case strings.Contains(f, "/models/query.py"):
			return "At call sites: select_related/prefetch_related + iterator(chunk_size=); do not list() huge QuerySets."
		case strings.Contains(f, "/template/base.py"):
			return "Profile expensive template filters/tags before ORM micro-opts; cache fragment output when safe."
		case strings.HasSuffix(f, "/server.c") || strings.HasSuffix(f, "/networking.c") || strings.HasSuffix(f, "/db.c") || strings.HasSuffix(f, "/acl.c"):
			return "Use SLOWLOG / latency-monitor; replace KEYS with SCAN; keep ACL rules tight — measure before micro-opts."
		case strings.Contains(f, "alloc") || strings.Contains(f, "buffer") || strings.Contains(base, "copy"):
			return "Prefer pooled/reused buffers or stack allocation on the hot path; avoid per-request heap growth."
		case strings.Contains(f, "parse") || strings.Contains(f, "compile"):
	...
	case security.ShapeSkeleton:
		return "Convention only — add pagination + eager-load (`with`/`select_related`) before treating this as a production hotspot."
	}
