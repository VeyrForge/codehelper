...
// Symbolic editing tools — codehelper's graph-driven rename/insert, optionally
// enriched with LSP references when a language server is installed. Without LSP
// the indexer uses conservative heuristics (~3% miss rate) and cannot see
// comments or string literals. Tools are PREVIEW-FIRST: every site is
// graph-confirmed / lsp-confirmed or textual-only. Applying requires apply=true;
// textual-only sites need include_textual=true.

const symbolEditHeuristicNote = "heuristic graph rename (optional LSP refs when installed); review graph/lsp-confirmed sites and treat textual-only as unverified before applying."
...
type renameSite struct {
	...
	Confidence string  `json:"confidence"` // graph-confirmed | lsp-confirmed | definition | textual-only
...
type renameSymbolResponse struct {
	...
	Source               string            `json:"source,omitempty"` // graph | graph+lsp
}
...
func renameSymbolHandler(reg *registry.Registry) server.ToolHandlerFunc {
	return func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var defLines []string
		if abs, aerr := absPathUnderRepo(repo.RootPath, def.Path); aerr == nil {
			defLines, _ = readFileLines(abs)
		}
		...
		for _, e := range edges {
			...
			var lines []string
			if abs, aerr := absPathUnderRepo(repo.RootPath, caller.Path); aerr == nil {
				lines, _ = readFileLines(abs)
			}
		...
		// 2b) Optional LSP references (when a language server is installed): merge
		//     type-precise sites the graph missed. Failures are silent — graph remains.
		lspUsed := enrichRenameWithLSP(ctx, repo.RootPath, *def, oldName, to, plans, ensure, confirmedKeys)
		...
		if !apply {
			...
			if lspUsed {
				out.Note += " LSP references merged where available (source=graph+lsp)."
			} else {
				out.Note += " LSP not used (missing server or CODEHELPER_LSP=0) — graph heuristic only."
			}
			out.RecommendedNextTools = []string{"impact", "context", "rename_symbol", "lsp"}
			out.Confidence = 0.7
...
func insertAtSymbolHandler(reg *registry.Registry) server.ToolHandlerFunc {
	return func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		// def.Path comes from the index, so re-validate it: an indexed symlink must not
		// be able to steer a read or write outside the repo.
		defRel, err := relativePathUnderRepo(repo.RootPath, def.Path)
		if err != nil {
			return mcp.NewToolResultError("definition file is not under the repository root: " + err.Error()), nil
		}
		if workspacePathBlocked(defRel, true) {
			return mcp.NewToolResultError("writes are blocked for this path (.git, node_modules, or .env*)"), nil
		}
		abs, err := absPathUnderRepo(repo.RootPath, def.Path)
		if err != nil {
			return mcp.NewToolResultError("definition file is not under the repository root: " + err.Error()), nil
		}
		...
		token, snapErr := snapshotPreEdit(repo.RootPath, defRel, []byte(strings.Join(lines, "")))
		if snapErr != nil {
			return mcp.NewToolResultError("snapshot failed: " + snapErr.Error()), nil
		}
		if err := atomicWriteUnderRepo(repo.RootPath, abs, []byte(updated)); err != nil {
			return mcp.NewToolResultError(err.Error()), nil
		}
		out.Applied = true
...
func applyRenameSites(root string, files []renameFilePlan, includeTextual bool) (int, []string, []string) {
	...
	for _, pl := range files {
		...
		// pl.Path comes from the index, so re-validate it: an indexed symlink must not
		// be able to steer a write outside the repo.
		rel, relErr := relativePathUnderRepo(root, pl.Path)
		if relErr != nil {
			warns = append(warns, pl.Path+": not under the repository root, skipped ("+relErr.Error()+")")
			continue
		}
		if workspacePathBlocked(rel, true) {
			warns = append(warns, pl.Path+": writes are blocked for this path, skipped")
			continue
		}
		abs, absErr := absPathUnderRepo(root, pl.Path)
		if absErr != nil {
			warns = append(warns, pl.Path+": not under the repository root, skipped ("+absErr.Error()+")")
			continue
		}
		...
		token, snapErr := snapshotPreEdit(root, rel, orig)
		if snapErr != nil {
			warns = append(warns, pl.Path+": snapshot failed, NOT written ("+snapErr.Error()+")")
			continue
		}
		if err := atomicWriteUnderRepo(root, abs, []byte(strings.Join(lines, ""))); err != nil {
...
// enrichRenameWithLSP merges optional LSP reference locations into the rename
// plan. Returns true when at least one LSP site was added. Failures are silent.
func enrichRenameWithLSP(
	ctx context.Context,
	root string,
	def types.Symbol,
	oldName, to string,
	plans map[string]*renameFilePlan,
	ensure func(string) *renameFilePlan,
	confirmedKeys map[string]bool,
) bool {
	if !lsp.Enabled() {
		return false
	}
	// Column of the identifier on the definition line (best-effort).
	col := 1
	if abs, aerr := absPathUnderRepo(root, def.Path); aerr == nil {
		if lines, err := readFileLines(abs); err == nil {
			if def.LineStart >= 1 && def.LineStart <= len(lines) {
				idx := strings.Index(lines[def.LineStart-1], oldName)
				if idx >= 0 {
					col = idx + 1
				}
			}
	...
	res, err := lsp.Query(ctx, root, lsp.Position{
		Path: def.Path, Line: def.LineStart, Col: col,
	}, lsp.ActionReferences)
	if err != nil || res == nil || res.Fallback || !res.OK || len(res.Locations) == 0 {
		return false
	}
	added := false
	for _, loc := range res.Locations {
		key := siteKey(loc.Path, loc.Line)
		if confirmedKeys[key] {
			continue
		}
		var lines []string
		if abs, aerr := absPathUnderRepo(root, loc.Path); aerr == nil {
			var rerr error
			lines, rerr = readFileLines(abs)
			if rerr != nil {
				continue
			}
		} else {
		...
		site, ok := siteOnLine(lines, loc.Line, oldName, to)
		if !ok {
			continue
		}
		confirmedKeys[key] = true
		site.Confidence = "lsp-confirmed"
		site.Score = 0.95
		ensure(loc.Path).GraphConfirmed = append(ensure(loc.Path).GraphConfirmed, site)
		added = true
	}
	_ = plans // plans mutated via ensure
	return added
}
