...
func QueryDB(ctx context.Context, repoRoot, connName, sqlText string, maxRows int) (*DBQueryResult, error) {
	...
	defer cancel()
	rows, tx, err := queryReadOnly(cctx, sqldb, driver, sqlText)
	if err != nil {
		return nil, err
	}
	if tx != nil {
...
// queryReadOnly runs sqlText. MySQL/Postgres use a read-only transaction when
// the driver supports it; SQLite relies on mode=ro in the DSN instead.
func queryReadOnly(ctx context.Context, sqldb *sql.DB, driver, sqlText string) (*sql.Rows, *sql.Tx, error) {
