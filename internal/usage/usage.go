...
// Preview collapses all whitespace runs to single spaces, redacts sensitive
// key/value material, and truncates to max runes (appending … when cut).
...
// sensitiveKeyRe matches common secret-bearing keys in JSON/query/form previews.
var sensitiveKeyRe = regexp.MustCompile(`(?i)"?(password|secret|token|authorization|cookie|api[_-]?key|dsn|connection[_-]?string)"?\s*([:=])\s*("[^"]*"|'[^']*'|[^\s,;}]+)`)

// RedactSensitive replaces values for password/secret/token/authorization/cookie/
// api_key/dsn/connection_string keys with [REDACTED] before telemetry previews.
func RedactSensitive(s string) string {
	if s == "" {
		return s
	}
	return sensitiveKeyRe.ReplaceAllString(s, `${1}${2}[REDACTED]`)
}
