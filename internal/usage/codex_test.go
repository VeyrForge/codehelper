...
func TestLoadCodexSessions(t *testing.T) {
	...
	// Marshal cwd so Windows backslashes are valid JSON escape sequences.
	cwdJSON, err := json.Marshal(repo)
	if err != nil {
		t.Fatal(err)
	}
	rollout := `{"timestamp":"2026-06-13T09:10:58.389Z","type":"session_meta","payload":{"id":"abc123","cwd":` + string(cwdJSON) + `,"source":"vscode"}}
