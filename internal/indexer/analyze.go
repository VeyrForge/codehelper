...
func Run(ctx context.Context, pathArg string, opt Options) error {
	...
	tProc := time.Now()
	if nProcs, nClus, err := st.PersistRequestFlows(ctx, repoID); err != nil {
		slog.Warn("request-flow clustering", "err", err)
	} else {
		slog.Info("request-flow clustering", "processes", nProcs, "clusters", nClus)
	}
...
func parseAndIngest(ctx context.Context, st *graph.Store, fc *cache.FileCache, repoID, indexRoot string, toReparse []string, symCount, edgeCount *int, progress func(done int, rel string)) error {
	...
	// Read tsconfig/vite/package.json path aliases once so alias imports
	// (@/lib/db, ~/components/Button) become repo-relative import edges that
	// symref resolution can match. Read-only after this point.
	aliases := DetectImportAliases(indexRoot)
	...
	for w := 0; w < nWorkers; w++ {
		...
		go func() {
			...
			for rel := range jobs {
				...
				res.Edges = ExpandAliasImportEdges(repoID, rel, res.Edges, aliases)
...
func languageFromExt(rel string) string {
	if lang := parser.DevOpsLanguage(rel); lang != "" {
		return lang
	}
	// *.blade.php is a Laravel template, not PHP — it has its own extractor.
	if parser.IsBladePath(rel) {
		return "blade"
	}
