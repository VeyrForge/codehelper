...
// TestWalkSourceFilesResolvesWindowsJunction ensures analyze roots staged as
// junctions (.testbeds/real-oss → .eval-projects) still yield source files.
// Without ResolveWalkRoot, WalkDir sees ModeIrregular and indexes 0 files.
func TestWalkSourceFilesResolvesWindowsJunction(t *testing.T) {
	if runtime.GOOS != "windows" {
		t.Skip("Windows junctions only")
	}
	target := t.TempDir()
	mustWrite(t, filepath.Join(target, "pkg", "a.py"), "x = 1\n")
	linkParent := t.TempDir()
	link := filepath.Join(linkParent, "bed")
	out, err := exec.Command("cmd", "/c", "mklink", "/J", link, target).CombinedOutput()
	if err != nil {
		t.Fatalf("mklink /J: %v (%s)", err, out)
	}
	got, err := WalkSourceFiles(link, nil, nil)
	if err != nil {
		t.Fatal(err)
	}
	joined := strings.Join(got, ",")
	if !strings.Contains(joined, "pkg/a.py") {
		t.Fatalf("junction walk missed sources: got %v (resolved=%s)", got, ResolveWalkRoot(link))
	}
}

// TestCollectNestedGitignorePaths_WindowsJunction ensures layered gitignore
// discovery follows junction roots (same ModeIrregular WalkDir gap as analyze).
func TestCollectNestedGitignorePaths_WindowsJunction(t *testing.T) {
	if runtime.GOOS != "windows" {
		t.Skip("Windows junctions only")
	}
	target := t.TempDir()
	mustWrite(t, filepath.Join(target, ".gitignore"), "*.log\n")
	mustWrite(t, filepath.Join(target, "pkg", ".gitignore"), "*.tmp\n")
	linkParent := t.TempDir()
	link := filepath.Join(linkParent, "bed")
	out, err := exec.Command("cmd", "/c", "mklink", "/J", link, target).CombinedOutput()
	if err != nil {
	...
	got, err := collectNestedGitignorePaths(link, nil)
	if err != nil {
		t.Fatal(err)
	}
	var foundPkg bool
	for _, p := range got {
		if strings.Contains(filepath.ToSlash(p), "/pkg/.gitignore") || strings.HasSuffix(filepath.ToSlash(p), "pkg/.gitignore") {
			foundPkg = true
			break
		}
	}
	if !foundPkg {
		t.Fatalf("junction gitignore walk missed pkg/.gitignore: got %v (resolved=%s)", got, ResolveWalkRoot(link))
	}
}
