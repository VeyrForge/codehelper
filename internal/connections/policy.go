...
type Policy struct {
	...
	// When empty/unset, DefaultVerifyAllowlist is used (never allow-all).
	VerifyAllowlist []string `json:"verify_allowlist,omitempty"`
	// VerifyAllowShell permits exec_mode=shell for local verify. Default false.
	// Only the user CLI may set this (agents cannot enable shell).
	// When enabled, every pipeline/compound segment executable must still be on
	// verify_allowlist; redirects and newlines remain blocked (not first-token-only).
	VerifyAllowShell bool `json:"verify_allow_shell,omitempty"`
...
// DefaultVerifyAllowlist is the auto toolchain set used when the project has not
// configured verify_allowlist. Empty allowlist never means allow-all.
var DefaultVerifyAllowlist = []string{
	"go", "npm", "npx", "pnpm", "yarn", "bun", "make", "cargo",
	"mvn", "gradle", "dotnet", "cmake", "ninja",
	"python", "python3", "pytest", "node", "php", "composer", "phpunit",
	"ruby", "bundle", "rake", "pip", "pip3", "uv", "tox",
	"jest", "vitest", "tsc", "eslint", "prettier", "rustc",
}
...
func (p Policy) IsConfigured() bool {
	p = NormalizePolicy(p)
	return p.AllowGit || p.VerifyAllowShell || len(p.VerifyAllowlist) > 0 || p.AgentTrust == AgentTrustAllowlistEdits ||
...
// EffectiveVerifyAllowlist returns the allowlist verify should use.
// Project VerifyAllowlist (or DefaultVerifyAllowlist when unset) is the cap;
// the request list can only narrow it — never widen. Empty intersection = deny.
func (p Policy) EffectiveVerifyAllowlist(requested []string) []string {
	p = NormalizePolicy(p)
	base := p.VerifyAllowlist
	if len(base) == 0 {
		base = append([]string(nil), DefaultVerifyAllowlist...)
	}
	if len(requested) == 0 {
...
func VerifyBlockPolicy(repoRoot string) verify.BlockPolicy {
	...
	return verify.BlockPolicy{
		AllowGit:   cfg.Policy.AllowGit,
		AllowShell: cfg.Policy.VerifyAllowShell,
	}
}

// ResolveVerifyAllowlist loads project policy and merges with a request allowlist.
// Always returns a non-empty default toolchain list when the project has no cap,
// so empty never means allow-all.
func ResolveVerifyAllowlist(repoRoot string, requested []string) []string {
	cfg, err := Load(repoRoot)
	if err != nil {
		return Policy{}.EffectiveVerifyAllowlist(requested)
	}
