...
var toolRefs = map[string]toolRef{
	...
	"kickoff":                    {summary: "One-call feature start: orient + reuse + impact + plan steps + verify + what_next.", params: "task · role=feature|… · sections"},
	...
	"context":                    {summary: "Source + callers + callees + blast radius for one symbol.", params: "name (NOT target) · body=brief|none|full"},
	"context_bundle":             {summary: "Bounded ACI bundle: source + callers + callees + imports + tests.", params: "name · max_callers · include_tests"},
	"impact":                     {summary: "Call-graph blast radius before changing a symbol.", params: "target (NOT name) · direction"},
	...
	"change_kit":                 {summary: "Pre-edit bundle: definition, call sites, tests, risk tier.", params: "target (NOT name)"},
	...
	"orchestrate":                {summary: "Guided investigation: classify task → tool chain → agent_brief + what_next.", params: "task · detail=true|false · format=toon|json"},
