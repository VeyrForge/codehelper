...
type finishRequest struct {
	...
	AllowClaimWithoutVerify bool   `json:"allow_claim_without_verify,omitempty"`
	DiagnosticsRan          bool   `json:"diagnostics_ran,omitempty"`
}
...
func runFinishCheck(ctx context.Context, root string, req finishRequest) (review.FinishCheckOutput, error) {
	...
	hasEditProof := false
	if files, derr := gitutil.DiffAgainst(abs, base); derr == nil {
		hasEditProof = len(files) > 0
	} else if base == "HEAD~1" {
		if files, derr2 := gitutil.DiffAgainst(abs, "HEAD"); derr2 == nil {
			hasEditProof = len(files) > 0
		}
	}
	return review.BuildFinishCheck(review.FinishCheckInput{
		...
		AllowClaimWithoutVerify: req.AllowClaimWithoutVerify,
		HasEditProof:            hasEditProof,
		DiagnosticsRan:          req.DiagnosticsRan,
		Release:                 rr,
	}), nil
}
