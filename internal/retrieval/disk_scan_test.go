...
func TestDiskGrepIdentifier_FollowsWindowsJunction(t *testing.T) {
	...
	mustWrite(t, filepath.Join(target, "def.go"), "package x\n\nfunc JunctionProbeSymbol() {}\n")
	cmd := exec.Command("cmd", "/c", "mklink", "/J", link, target)
	if out, err := cmd.CombinedOutput(); err != nil {
		t.Skipf("mklink failed: %v (%s)", err, out)
	}
	matches := DiskGrepIdentifier(link, "JunctionProbeSymbol", 5)
	if len(matches) == 0 {
		t.Fatal("junction disk grep returned 0 matches — WalkDir missed sources")
	}
}
