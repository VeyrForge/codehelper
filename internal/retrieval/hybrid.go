...
type QueryOptions struct {
	...
	// queryMode retains explore|debug|test|refactor when Intent is rewritten to the
	// locate query for Type.Method / name ranking (see QueryHybridWithOptions).
	queryMode string
}
...
func QueryHybridWithOptions(ctx context.Context, st *graph.Store, repoID, q string, limit int, opts QueryOptions) ([]RankedSymbol, error) {
	...
	// Type.Method / Recv.Method boosts need the raw locate string (dots preserved).
	// MCP often sets Intent to explore|debug|test|refactor, and kickoff historically
	// passed Intent=""; without falling back to q, Context.JSON loses to SecureJsonPrefix.
	// Keep opts.Intent intact for applyQueryIntentBoosts (mode labels), and stash the
	// mode on rankOpts.queryMode so rerankWithSignals still applies nearest_test boosts
	// after Intent is rewritten to q.
	rankOpts := opts
	if iq := strings.TrimSpace(opts.Intent); iq == "" || isQueryModeIntent(iq) ||
		(strings.Contains(q, ".") && !strings.Contains(iq, ".")) {
		if isQueryModeIntent(iq) {
			rankOpts.queryMode = strings.ToLower(iq)
		}
		rankOpts.Intent = q
	}

	fused := rerankWithSignals(bm25List, rankOpts)
...
// isQueryModeIntent reports MCP intent= explore|debug|test|refactor labels that
// must not replace the locate query for Type.Method ranking.
func isQueryModeIntent(s string) bool {
	switch strings.ToLower(strings.TrimSpace(s)) {
	case "explore", "debug", "test", "refactor":
		return true
	default:
		return false
	}
