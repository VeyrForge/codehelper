...
func embedAutoDetectEnabled() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CODEHELPER_EMBED_AUTO")))
...
// EnsureEmbedder lazily wires the opt-in semantic backend from (in order):
//  1. an already-installed embedder / explicit CODEHELPER_EMBED_URL
//  2. ~/.codehelper/green.json via green.LoadAndExport (when auto-detect is on)
//  3. a localhost probe of common ge embed ports (when auto-detect is on)
//
// Called from MCP query paths only — not from QueryHybridWithOptions directly —
// so unit tests stay deterministic. Set CODEHELPER_EMBED_AUTO=0 to skip steps 2–3.
// Never downloads models; probing is a cheap GET /v1/models.
func EnsureEmbedder() {
	...
	if u := probeLocalEmbedServer(); u != "" {
		activeEmbedder = newHTTPEmbedder(u)
		_ = os.Setenv("CODEHELPER_EMBED_URL", u)
	}
}
