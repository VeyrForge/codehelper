...
func TestSplitQualifiedQueryAndModeIntent(t *testing.T) {
	recv, method, ok := splitQualifiedQuery("Context.JSON")
	if !ok || recv != "Context" || method != "JSON" {
		t.Fatalf("Context.JSON → %q.%q ok=%v", recv, method, ok)
	}
	if !isQueryModeIntent("explore") || !isQueryModeIntent("DEBUG") {
		t.Fatal("explore/debug should be mode intents")
	}
	if isQueryModeIntent("Context.JSON") || isQueryModeIntent("add health") {
		t.Fatal("locate strings must not be mode intents")
	}
}

func TestQueryHybrid_ContextJSONPrefersQualifiedRecv(t *testing.T) {
	// Synthetic pool mirrors the gin failure mode: SecureJsonPrefix BM25-tops
	// Context.JSON until qualified_recv boost applies via Intent=q fallback.
	in := []RankedSymbol{
		{Symbol: types.Symbol{ID: "1", Name: "SecureJsonPrefix", Kind: "method", ParentID: "Engine", Path: "gin.go"}, Score: 1.0, Reasons: []string{"bm25"}},
		{Symbol: types.Symbol{ID: "2", Name: "JSON", Kind: "method", ParentID: "Context", Path: "context.go"}, Score: 0.91, Reasons: []string{"bm25"}},
		{Symbol: types.Symbol{ID: "3", Name: "JsonpJSON", Kind: "function", Path: "render/json.go"}, Score: 0.92, Reasons: []string{"bm25"}},
	}
	out := rerankWithSignals(append([]RankedSymbol(nil), in...), QueryOptions{
		Intent: "Context.JSON", QueryTokens: tokenize("Context.JSON"),
	})
	if len(out) == 0 || out[0].Symbol.Name != "JSON" || out[0].Symbol.ParentID != "Context" {
		t.Fatalf("top=%v want Context.JSON", summarizeRanked(out, 3))
	}
	found := false
	...
	if !found {
		t.Fatalf("missing qualified_recv on top: %v", out[0].Reasons)
	}
}

func summarizeRanked(hits []RankedSymbol, n int) []string {
	if n > len(hits) {
		n = len(hits)
	}
	out := make([]string, 0, n)
	for i := 0; i < n; i++ {
		out = append(out, hits[i].Symbol.ParentID+"."+hits[i].Symbol.Name+"@"+hits[i].Symbol.Path)
	}
	return out
}
