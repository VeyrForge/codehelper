...
// Markdown tables mentioning "form requests" + "Model::where()" must not look
// like a foreach+ORM N+1 (loopStart historically matched the "for" in "form").
func TestPerfGuard_SkipsMarkdownTableFalseFriends(t *testing.T) {
	dir := newGitFixture(t)
	body := "| Laravel | High | via PHP / Blade | Routes → controller actions (middleware, form requests); Model::where() entries |\n"
	writeAndStage(t, dir, "docs/LANGUAGE_MATRIX.md", body)
	res, err := PerfGuard(context.Background(), dir, "HEAD~1")
	if err != nil {
		t.Fatal(err)
	}
	if len(res.PerfRisks) != 0 {
		t.Fatalf("markdown densify table must not be N+1, got %#v", res.PerfRisks)
	}
}

// Even inside a production .go string, "form requests" must not open a loop window.
func TestPerfGuard_FormRequestsNotLoopStart(t *testing.T) {
	dir := newGitFixture(t)
	writeAndStage(t, dir, "internal/docs/matrix.go", "package docs\n\nconst row = `| Laravel | form requests); Model::where() |`\n")
	res, err := PerfGuard(context.Background(), dir, "HEAD~1")
	if err != nil {
		t.Fatal(err)
	}
	if len(res.PerfRisks) != 0 {
		t.Fatalf("form≠for in Go string must not be N+1, got %#v", res.PerfRisks)
	}
}

func TestLoopStart_RejectsFormFalseFriend(t *testing.T) {
	if loopStart.MatchString("middleware, form requests); Model::where()") {
		t.Fatal("loopStart must not match form requests")
	}
	if !loopStart.MatchString("foreach ($users as $user)") {
		t.Fatal("loopStart must still match foreach")
	}
	if !loopStart.MatchString("for i := 0; i < n; i++") {
		t.Fatal("loopStart must still match Go for")
	}
}
