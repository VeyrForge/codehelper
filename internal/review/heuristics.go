...
func IsCodeSourceFile(path string) bool {
	...
	// Fixture/demo trees are intentional probes, not production contracts —
	// do not demand sibling regression tests or flag them as public API breaks.
	if IsSecondaryNoisePath(p) {
		return false
	}
	...
	for _, prefix := range []string{"out/", "dist/", "build/", "target/", "bin/", "coverage/", "third_party/"} {
...
// IsEngineAddonPath reports Godot addons/, Unity Library/ cache, and Unreal
// Plugins/ trees that collide on ubiquitous lifecycle names (_ready, _process,
// BeginPlay) with first-party scripts. Indexed and searchable; query/context/impact
// should demote them so agents prefer game scripts without path= when a unique
// production hit remains.
func IsEngineAddonPath(path string) bool {
	p := strings.ToLower(filepath.ToSlash(path))
	if p == "" {
		return false
	}
	for _, seg := range []string{"/addons/", "/library/", "/plugins/"} {
		if strings.Contains(p, seg) {
			return true
		}
	}
	for _, prefix := range []string{"addons/", "library/", "plugins/"} {
		if strings.HasPrefix(p, prefix) {
			return true
		}
	}
