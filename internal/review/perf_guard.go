...
// Trailing \b after keywords avoids false friends like "form" / "format"
// (historically matched as "for" + word char with zero whitespace).
var loopStart = regexp.MustCompile(`(?i)\b(foreach|for|while|do)\b\s*(\(|\$|\w|:)|\bforEach\s*\(`)

// inLoopQuery matches database/ORM/HTTP/IO calls that imply N+1 or unbounded I/O
// when they appear inside a loop window.
var inLoopQuery = regexp.MustCompile(`(?i)->(get|find|first|firstOrFail|where|all|load|with|count|exists|paginate|chunk|fresh)\s*\(|::(find|where|all|first|firstOrFail|with|load|count|paginate)\s*\(|\bDB::(table|select|insert|update|delete)\s*\(|\bobjects\.(get|filter|all|first)\s*\(|\.(find_by|find_by_id|find_each)\s*\(|\bquery\s*\(|\bfindOne\s*\(|\bfindAll\s*\(|\bfindMany\s*\(|\bgetRepository\s*\(|\baxios\.(get|post|put|delete)\s*\(|\bfetch\s*\(|\.scan\s*\(|\.bytes\s*\(|requests\.(get|post|put|delete)\s*\(|\bfile_get_contents\s*\(|\bfopen\s*\(|\bprisma\.\w+\.(findMany|findFirst|findUnique)\s*\(`)
...
func PerfGuard(ctx context.Context, repoRoot, baseRef string) (*PerfGuardResult, error) {
	...
	for _, f := range files {
		if !IsCodeSourceFile(f.Path) || IsTestPath(f.Path) {
			continue
		}
