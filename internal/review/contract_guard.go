...
func likelyPublic(path, name string) bool {
	...
	// Go export rules are authoritative: unexported helpers under cmd/pkg/api
	// are package-private and must not ship-block as "public contract" breaks.
	if strings.HasSuffix(p, ".go") {
		return exported
	}

	// Public surface heuristics (non-Go): capitalized symbols OR symbols living
	// in canonical "shared" / "API" locations.
	return exported ||
