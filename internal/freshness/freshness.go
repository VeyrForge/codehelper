...
func Inspect(indexRoot string) Report {
	...
	// Parser/schema bumps change extraction without touching git HEAD or file
	// mtimes. Analyze already full-reindexes on mismatch; surface it here so MCP
	// tools do not report a false-fresh index after a binary upgrade.
	if !r.Stale && m != nil && versionMismatch(m) {
		r.Stale = true
		r.StaleReason = "parser/schema version mismatch (index older than this binary)"
		r.SuggestedFix = "codehelper analyze (reindex required after upgrade; restart watch if a daemon is running)"
	}
	...
	if r.Stale && r.WatchRunning && !versionMismatchReason(r.StaleReason) {
...
func versionMismatch(m *meta.Data) bool {
	if m == nil {
		return false
	}
	...
	// ParserVersion 0 is unset (legacy fixtures / partial meta writes). Real
	// analyze always stamps parser.Version; only non-zero drift is actionable.
	if m.ParserVersion != 0 && m.ParserVersion != parser.Version {
		return true
	}
	return false
}

func versionMismatchReason(reason string) bool {
	r := strings.ToLower(reason)
	return strings.Contains(r, "parser") || strings.Contains(r, "schema version")
}
...
func attachActionRequired(r *Report) {
	...
	if r.Stale {
		...
		// Watch converges on file/HEAD drift, but NOT on parser/schema bumps (a
		// long-lived daemon keeps the old binary's extractors until restarted).
		if r.WatchRunning && r.StaleReason != "no index" && !versionMismatchReason(r.StaleReason) {
			ar.Commands = nil
