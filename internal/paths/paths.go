...
// On Windows the hash input is Canonical (slash + lower) so F:\App and f:\app
// share one external index instead of silently forking.
func repoKey(repoRoot string) string {
	...
	if runtime.GOOS == "windows" {
		keyPath = Canonical(abs)
		base = strings.ToLower(base)
	}
	sum := sha1.Sum([]byte(keyPath))
