...
// Query runs a one-shot hover/definition/references request. It starts the
// language server, opens the file, issues the request, then shuts down.
func Query(ctx context.Context, repoRoot string, pos Position, action Action) (*Result, error) {
	...
	default:
		return nil, fmt.Errorf("unknown action %q (use hover|definition|references)", action)
	}
