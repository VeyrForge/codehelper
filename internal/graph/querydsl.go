...
// CallersOf returns symbols with a calls, reads, inherits, or implements edge
// pointing to calleeID. Reads catch module/factory references (JS/TS CJS);
// inherits/implements densify PHP/Ruby/C# class inbound so subclass/mixin
// edges count for context/impact (not just calls).
...
WHERE e.repo_id = ? AND e.dst_id = ? AND e.kind IN (?, ?, ?, ?)`,
		repoID, calleeID,
		string(types.RefKindCalls), string(types.RefKindReads),
		string(types.RefKindInherits), string(types.RefKindImplements))
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
FROM edges e JOIN symbols s ON s.id = e.src_id AND s.repo_id = e.repo_id
WHERE e.repo_id = ? AND e.dst_id = ? AND e.kind IN (?, ?, ?, ?)
LIMIT ?`, repoID, calleeID,
		string(types.RefKindCalls), string(types.RefKindReads),
		string(types.RefKindInherits), string(types.RefKindImplements), limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
// reads, inherits, or implements — an index-backed COUNT, so a consumer can
// report "N callers (showing 12)" without materializing all N.
func (s *Store) CountCallers(ctx context.Context, repoID, calleeID string) (int, error) {
	...
	err := s.db.QueryRowContext(ctx,
		...
		repoID, calleeID,
		string(types.RefKindCalls), string(types.RefKindReads),
		string(types.RefKindInherits), string(types.RefKindImplements)).Scan(&n)
	return n, err
}
