...
func TestCallersOf_InheritsAndImplements(t *testing.T) {
	ctx := context.Background()
	st, err := Open(filepath.Join(t.TempDir(), "inh.db"))
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() { _ = st.Close() })
	const repo = "r"
	base := "sym:r:base.php:1:Base"
	user := "sym:r:user.php:1:User"
	aud := "sym:r:aud.php:1:Auditable"
	for _, s := range []types.Symbol{
		{ID: base, RepoID: repo, Name: "Base", Kind: types.SymbolKindClass, Path: "base.php", LineStart: 1},
		{ID: user, RepoID: repo, Name: "User", Kind: types.SymbolKindClass, Path: "user.php", LineStart: 1},
		{ID: aud, RepoID: repo, Name: "Auditable", Kind: types.SymbolKindInterface, Path: "aud.php", LineStart: 1},
	} {
		if err := st.UpsertSymbol(ctx, s); err != nil {
			t.Fatal(err)
		}
	...
	if err := st.AddEdge(ctx, types.Reference{
		ID: "e1", RepoID: repo, Kind: types.RefKindInherits, SourceID: user, TargetID: "symref:r:user.php:Base", Confidence: 0.9,
	}); err != nil {
		t.Fatal(err)
	}
	if err := st.AddEdge(ctx, types.Reference{
		ID: "e2", RepoID: repo, Kind: types.RefKindImplements, SourceID: user, TargetID: "symref:r:user.php:Auditable", Confidence: 0.9,
	}); err != nil {
		t.Fatal(err)
	}
	...
	if stats.Resolved < 2 {
		t.Fatalf("expected inherits+implements resolved, got %+v", stats)
	}
	callers, err := st.CallersOf(ctx, repo, base)
	if err != nil {
		t.Fatal(err)
	}
	if len(callers) != 1 || callers[0].ID != user {
		t.Fatalf("Base callers via inherits want User, got %#v", callers)
	}
	implCallers, err := st.CallersOf(ctx, repo, aud)
	if err != nil {
		t.Fatal(err)
	}
	if len(implCallers) != 1 || implCallers[0].ID != user {
		t.Fatalf("Auditable callers via implements want User, got %#v", implCallers)
	}
}
