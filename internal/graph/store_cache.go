...
// CloseCached drops and immediately closes any process-cached Store for dbPath.
// Tests that auto-index into a TempDir must call this before cleanup so Windows
// can unlink graph.db (shared OpenCached handles otherwise keep the file locked).
func CloseCached(dbPath string) error {
	storeCacheMu.Lock()
	defer storeCacheMu.Unlock()
	e, ok := storeCache[dbPath]
	if !ok || e == nil || e.store == nil {
		return nil
	}
	delete(storeCache, dbPath)
	return e.store.closeReal()
}
