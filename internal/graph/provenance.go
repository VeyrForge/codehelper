...
func ProvenanceForStrategy(strategy string) Provenance {
	...
	case "import", "recv_type":
		return Exact
	case "same_file", "same_dir", "same_subtree", "public_api", "embedded", "type_kind":
		return Scoped
	case "unique", "non_fixture", "non_vendor", "impl_over_decl":
		return NameOnly
	default:
		return Inferred
	}
...
// ProvenanceFromConfidence maps a stored edge confidence back to a band label
// for agent-facing impact/context payloads.
func ProvenanceFromConfidence(c float64) Provenance {
	switch {
	case c >= ConfExact:
		return Exact
	case c >= ConfScoped:
