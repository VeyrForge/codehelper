...
// isolateHome redirects os.UserHomeDir away from the real profile.
// Unix uses HOME; Windows prefers USERPROFILE (HOME alone is ignored).
...
// sameCanonicalPath compares paths after Abs/Clean/EvalSymlinks so Windows 8.3
// short names (GREENE~1) match their long-form equivalents.
func sameCanonicalPath(a, b string) bool {
	return resolveAllowRoot(a) == resolveAllowRoot(b)
}
...
func TestBrowserConfigEnvOverride(t *testing.T) {
	...
	if !sameCanonicalPath(gotDir, wantDir) {
		t.Fatalf("config path dir = %q want %q", gotDir, wantDir)
	}
}
