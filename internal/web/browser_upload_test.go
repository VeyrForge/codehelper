...
func TestResolveUploadPathsSymlinkedRoot(t *testing.T) {
	base := t.TempDir()
	realRoot := filepath.Join(base, "real")
	if err := os.MkdirAll(realRoot, 0o755); err != nil {
		t.Fatal(err)
	}
	inside := filepath.Join(realRoot, "ok.txt")
	...
	for name, root := range map[string]string{
		...
		if err != nil || len(paths) != 1 {
			t.Fatalf("%s: inside file: %v %#v", name, err, paths)
		}
	}

	outside := t.TempDir()
	secret := filepath.Join(outside, "secret.txt")
	if err := os.WriteFile(secret, []byte("y"), 0o644); err != nil {
		t.Fatal(err)
	}
	external := filepath.Join(realRoot, "external")
	if err := os.Symlink(outside, external); err != nil {
		t.Skipf("symlinks not available: %v", err)
	}
	escaped := filepath.Join(external, "secret.txt")
	_, err := ResolveUploadPaths(escaped, realRoot, nil)
