...
// resolveAllowRoot canonicalizes a sandbox root the same way upload targets are
// resolved: Abs/Clean plus EvalSymlinks when the path exists. On Windows this
// expands 8.3 short names (e.g. GREENE~1) so containment checks match targets
// that EvalSymlinks already normalized.
func resolveAllowRoot(d string) string {
