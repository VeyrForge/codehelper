...
// DefaultDepth is the shared BFS depth for impact and context blast_radius
// so risk_tier agrees across those tools for the same symbol.
const DefaultDepth = 2

// RiskTierFromNodeCount is the single source of truth for impact/context risk_tier.
// Thresholds are inclusive of the target node (depth 0).
func RiskTierFromNodeCount(n int) string {
	switch {
	case n > 40:
		return "high"
	case n > 10:
...
func Analyze(ctx context.Context, st *graph.Store, repoID, target string, maxDepth int, direction string) (*types.ImpactResult, error) {
	...
	for len(queue) > 0 {
		...
		neighbors, err := st.Neighbors(ctx, repoID, cur.id, incoming,
			string(types.RefKindCalls), string(types.RefKindReads), string(types.RefKindImports),
	...
	risk := RiskTierFromNodeCount(len(nodes))
