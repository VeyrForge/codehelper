...
var (
	...
	// Require a DB/query API OR SQL-verb+concat on the sink line. Do NOT treat bare
	// `${` templates as SQL sinks (Response/status template FPs); HTTP response
	// helpers are demoted via looksLikeSQL → isHTTPOrUIConcat.
...
func detectRequestToSinkTaint(rel string, lines []string, remaining int) []ContextFinding {
	...
	for _, fr := range funcs {
		...
		for _, l := range body {
			if isRelativeOriginURLAssign(l) {
				// new URL('/login', req.url) — same-origin relative; not an open redirect source.
				continue
			}
...
// isRelativeOriginURLAssign reports new URL('/path', req.url) / new URL("/path", req.url)
// constructions — same-origin relative redirects (Next.js middleware), not open redirects.
func isRelativeOriginURLAssign(line string) bool {
...
func lineUsesTainted(lower string, tainted map[string]bool) bool {
	for id := range tainted {
		...
		// Require identifier boundaries — "code" must not match "no_code" /
		// "invalid_state" (discord_mod PublicURL+"/?error=…" LIVE FPs).
		if containsIdent(lower, id) {
			return true
		}
	}
	return false
...
func containsIdent(lower, id string) bool {
	start := 0
	for {
		rel := strings.Index(lower[start:], id)
		if rel < 0 {
			return false
		}
		i := start + rel
