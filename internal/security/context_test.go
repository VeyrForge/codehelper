...
func TestScanDiffForSecuritySmells_SkipsNoisePaths(t *testing.T) {
	lines := []AddedDiffLine{
		{File: "internal/security/reposcan_test.go", Line: 99, Content: `const sql = "SELECT * FROM users WHERE name = '" + name + "'"`},
		{File: "app/db.go", Line: 4, Content: `query := "SELECT * FROM users WHERE id=" + id`},
	}
	got := ScanDiffForSecuritySmells(lines)
	for _, f := range got {
		if strings.Contains(strings.ToLower(f.File), "_test.go") || strings.Contains(f.File, "internal/security/") {
			t.Fatalf("noise-path finding should be skipped: %+v", f)
		}
	}
	foundApp := false
	for _, f := range got {
		if f.File == "app/db.go" && f.Rule == "sql-string-concat" {
			foundApp = true
		}
	}
	if !foundApp {
		t.Fatalf("expected production sql-string-concat on app/db.go, got %#v", got)
	}
}
