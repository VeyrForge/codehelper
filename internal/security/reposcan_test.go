...
// Builtin regex + repoExtra substring both match api_key=/password= — one cite per line.
func TestScanRepoForSecuritySmells_NoDuplicateHardcodedSecret(t *testing.T) {
	dir := t.TempDir()
	p := filepath.Join(dir, "app", "secrets.ts")
	if err := os.MkdirAll(filepath.Dir(p), 0o755); err != nil {
		t.Fatal(err)
	}
	body := "const api_key = \"probe_fixture_api_key_not_real\";\nconst password = \"SuperSecretFixtureValue99\";\n"
	if err := os.WriteFile(p, []byte(body), 0o644); err != nil {
		t.Fatal(err)
	}
	got := ScanRepoForSecuritySmells(dir, RepoScanOptions{Limit: 20})
	...
	for _, f := range got {
		...
		k := f.File + ":" + strconv.Itoa(f.Line)
		seen[k]++
		if seen[k] > 1 {
			t.Fatalf("duplicate hardcoded-secret at %s (count=%d)", k, seen[k])
		}
	}
	if len(seen) < 2 {
		t.Fatalf("expected secrets on both lines, got %+v", seen)
	}
}

// Lexical sql-string-concat + dataflow injection-taint on the same line → one cite.
func TestScanRepoForSecuritySmells_NoDuplicateSQLSibling(t *testing.T) {
	dir := t.TempDir()
	p := filepath.Join(dir, "app", "api", "route.ts")
	if err := os.MkdirAll(filepath.Dir(p), 0o755); err != nil {
		t.Fatal(err)
	}
	body := "" +
	...
	got := ScanRepoForSecuritySmells(dir, RepoScanOptions{Limit: 20})
	sqlLines := map[int][]string{}
	for _, f := range got {
		if f.Rule != "sql-string-concat" && f.Rule != "injection-taint" {
			continue
		}
		sqlLines[f.Line] = append(sqlLines[f.Line], f.Rule)
	}
	for line, rules := range sqlLines {
		if len(rules) > 1 {
			t.Fatalf("double-scan SQL siblings at line %d: %v", line, rules)
		}
	}
	if len(sqlLines) == 0 {
		t.Fatalf("expected one SQL/taint cite, got %+v", got)
	}
}

// requireAuth:false inside NextResponse.json is not an authz-gap; route = { auth: false } is.
func TestScanRepoRejectsAuthFlagInJSONResponse(t *testing.T) {
	dir := t.TempDir()
	mustWrite := func(rel, body string) {
		p := filepath.Join(dir, rel)
		if err := os.MkdirAll(filepath.Dir(p), 0o755); err != nil {
	...
	mustWrite("app/api/echo/route.ts", ""+
		"export async function GET() {\n"+
		"  return NextResponse.json({ sql: \"x\", requireAuth: false })\n"+
		"}\n")
	...
	for _, f := range got {
		...
		if strings.Contains(f.File, "echo") {
			t.Fatalf("JSON requireAuth:false must not be authz-gap: %+v", f)
		}
		if strings.Contains(f.File, "public.ts") {
			sawRoute = true
		}
	}
	if !sawRoute {
		t.Fatal("expected auth: false route config to remain authz-gap")
	}
}
...
func TestLooksLikeSQL_RejectsImports(t *testing.T) {
	...
	if looksLikeSQL(strings.ToLower(`this.send('SELECT * FROM users WHERE id=' + id)`)) {
		t.Fatal("prototype this.send must not look like SQL")
	}
	if looksLikeSQL(strings.ToLower(`reply.send('SELECT * FROM users WHERE id=' + id)`)) {
		t.Fatal("fastify reply.send must not look like SQL")
	}
	if looksLikeSQL(strings.ToLower(`w.write([]byte("SELECT * FROM users WHERE id=" + id))`)) {
		t.Fatal("ResponseWriter Write must not look like SQL")
	}
...
func TestScanRepoRejectsExpressHTTPConcatFPs(t *testing.T) {
	...
	// Prototype / Fastify aliases + Go ResponseWriter — SQL-looking prose is still HTTP.
	mustWrite("app/proto_send.js", "app.get('/x', function(req, res) {\n  const id = req.params.id;\n  this.send('SELECT * FROM users WHERE id=' + id);\n});\n")
	mustWrite("app/reply_send.js", "app.get('/x', function(req, reply) {\n  const id = req.params.id;\n  reply.send('SELECT * FROM users WHERE id=' + id);\n});\n")
	mustWrite("app/write_echo.go", "func h(w http.ResponseWriter, r *http.Request) {\n  id := r.URL.Query().Get(\"id\")\n  w.Write([]byte(\"SELECT * FROM users WHERE id=\" + id))\n}\n")
	...
	for _, f := range got {
		...
		case strings.Contains(f.File, "cookie-sessions"), strings.Contains(f.File, "pet.js"),
			...
			strings.Contains(f.File, "params.js"), strings.Contains(f.File, "proto_send"),
			strings.Contains(f.File, "reply_send"), strings.Contains(f.File, "write_echo"):
			t.Fatalf("express HTTP concat must not be %s: %+v", f.Rule, f)
		}
	}
	sawSQL := false
	for _, f := range got {
...
func TestScanRepoRejectsDrizzleParameterizedSQL(t *testing.T) {
	...
	mustWrite("app/api/feeds/sync/batch.ts", ""+
		"const values = gtins.map(g => `('${g.replace(/'/g, \"''\")}')`).join(',');\n"+
	...
	var sawDyn, sawClassic, sawRawVar bool
	for _, f := range got {
		if f.Rule != "sql-string-concat" {
			continue
		}
		if strings.Contains(f.File, "sync/route") {
			sawDyn = true
		}
		if strings.Contains(f.File, "classic.py") {
			sawClassic = true
		}
		if strings.Contains(f.File, "sync/batch") {
			sawRawVar = true
		}
	}
	if !sawDyn {
	...
	if !sawRawVar {
		t.Fatal("expected sql.raw(values) dynamic call to remain")
	}
}
...
func TestScanRepoFindsInsecureCookieAndTemplateCSS(t *testing.T) {
	dir := t.TempDir()
	mustWrite := func(rel, body string) {
		p := filepath.Join(dir, rel)
		if err := os.MkdirAll(filepath.Dir(p), 0o755); err != nil {
	...
	mustWrite("backend/internal/auth/handler.go", ""+
		"func (h *Handler) setSession(w http.ResponseWriter, r *http.Request, sessionID string) {\n"+
		"\tsecure := false\n"+
		"\tif r != nil && r.TLS != nil { secure = true }\n"+
		"\thttp.SetCookie(w, &http.Cookie{\n"+
		"\t\tName: \"sid\", Value: sessionID, Path: \"/\",\n"+
		"\t\tHttpOnly: true, Secure: secure, SameSite: http.SameSiteLaxMode,\n"+
		"\t})\n}\n")
	mustWrite("backend/pkg/cardrender/themed_rank.go", ""+
		"data := themedRankCardData{\n"+
		"\tShellStyle: template.CSS(themedRankShellStyle(backgroundValue, accent)),\n"+
		"\tAvatarStyle: template.CSS(fmt.Sprintf(\"width:%dpx;height:%dpx;\", avatarSizePx, avatarSizePx)),\n"+
		"\tProgressFill: template.CSS(progressFill),\n"+
		"}\n")
	mustWrite("pkg/static_style.go", "style := template.CSS(\"color: red\")\n")
	mustWrite("pkg/flags.go", "secure := false // TLS toggle for dialer, not a cookie\n")

	got := ScanRepoForSecuritySmells(dir, RepoScanOptions{Limit: 40})
	var sawCookie, sawCSS, sawStaticCSS, sawFlag, sawNumericCSS bool
	for _, f := range got {
		switch f.Rule {
		case "insecure-cookie":
			if strings.Contains(f.File, "handler.go") {
				sawCookie = true
			}
			if strings.Contains(f.File, "flags.go") {
				sawFlag = true
			}
		case "unsafe-template-css":
			if strings.Contains(f.Evidence, "themedRankShellStyle") || strings.Contains(f.Evidence, "progressFill") {
				sawCSS = true
			}
			if strings.Contains(f.File, "static_style") {
				sawStaticCSS = true
			}
			if strings.Contains(f.Evidence, "width:%dpx") || strings.Contains(f.Evidence, "fmt.sprintf") {
				sawNumericCSS = true
			}
		}
	}
	if !sawCookie {
		t.Fatal("expected secure := false before SetCookie to remain")
	}
	if !sawCSS {
		t.Fatal("expected template.CSS(variable) to remain")
	}
	if sawStaticCSS {
		t.Fatal("template.CSS(\"literal\") must not be a sink")
	}
	if sawNumericCSS {
		t.Fatal("template.CSS(fmt.Sprintf numeric-only) must not be a sink")
	}
	if sawFlag {
		t.Fatal("non-cookie secure := false must not be a sink")
	}
}
