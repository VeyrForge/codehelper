...
func ScanDiffForSecuritySmells(lines []AddedDiffLine) []ContextFinding {
	...
	for _, line := range lines {
		// Align with ScanRepoForSecuritySmells: never score test/fixture/scanner
		// noise paths as ship-blocking sinks (intentional probe strings in *_test.go).
		if isRepoScanNoisePath(line.File) {
			continue
		}
