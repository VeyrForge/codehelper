...
func DetectProjectShape(root string) ProjectShape {
	...
	// Resolve after basename so junction link names (laravel, express) keep
	// shape heuristics while WalkDir counts see the real tree.
	...
	// Vue/Svelte: layout-only (packages/*) — basename "vue"/"svelte" also matches
	// thin SPA beds (Button.vue / Page.svelte) which must stay ShapeApp.
	frameworkCores := map[string]bool{
		"express": true, "django": true, "flask": true, "fastapi": true,
		"gin": true, "rails": true, "axum": true, "redis": true,
		"spring-petclinic": false, // app demo, not spring-core
	}
	...
	// Thin Vue/Svelte SPA apps before lib/ → library (Svelte kits often use lib/).
	if isFrontendSpaApp(root) {
		return ShapeApp
	}
