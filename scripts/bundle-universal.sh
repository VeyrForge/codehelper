...
#
# A "universal" artifact requires BOTH amd64 and arm64. Missing both arches
# skips that OS. A single arch also skips (never publish a mislabeled universal)
# so release can still ship per-arch archives when experimental arm64 flakes.
#
...
bundle_unix_os() {
  ...
  if [ "$count" -ne 2 ]; then
    echo "Skip ${os} universal: need both amd64 and arm64 (found $count arch(es) in $DIST_ROOT); per-arch archives still ship" >&2
    rm -rf "$out_dir"
    return 0
  fi
...
bundle_windows() {
  ...
  if [ "$count" -ne 2 ]; then
    echo "Skip windows universal: need both amd64 and arm64 (found $count arch(es) in $DIST_ROOT); per-arch zips still ship" >&2
    rm -rf "$out_dir"
    return 0
  fi
