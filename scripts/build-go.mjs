...
// Product modules (docs/MODULES.md): default full bundle includes -tags rod.
// Selective installs: CODEHELPER_MODULES=core,edit,ops (adds ch_modules + ch_*).
let moduleTags;
try {
  moduleTags = resolveModules(process.env.CODEHELPER_MODULES).tags;
} catch (err) {
  console.error(err instanceof Error ? err.message : String(err));
  process.exit(1);
}
const buildTags = tagsToGoArgs(moduleTags);
