...
- **Bundled `ge` / `greencompress` 1.1.1** — Cargo.toml + shipping docs/overlays claim
  **1.1.1** (matches on-disk crate versions). Subtree pin SHAs are still the prior tip;
  **blocker:** after standalone greens commit 1.1.1, re-subtree + prune so pins lockstep.
  Codehelper product version remains **3.0.3**.

## 3.0.3

### Changed (license — BUSL-1.1 grant clarification)

- **Clarified BUSL-1.1 Additional Use Grant** — personal and internal business
  production use (including employees/contractors) is explicitly allowed; the
  restriction targets offering Codehelper (or a substantially similar substitute)
  to third parties as a hosted, managed, embedded, or distributed product or
  service. Paid client work and using Codehelper on customer software remain
  allowed when customers are not offered Codehelper itself as a product/service.
- **Licensed Work / Change Date** — Licensed Work describes repo source and docs
  (excluding separately licensed components); Change Date **2029-07-24** →
  Apache-2.0 (or fourth anniversary of first public BSL distribution of that
  version, whichever is earlier). Licensor/brand: **VeyrForge**.
- **LICENSE-FAQ.md**, **CLA.md**, and `licensing@veyrforge.com` for commercial
  inquiries and contributor relicensing rights.

### Fixed

- **Docs cross-links** — MCP browser-ui-eval → `docs/browser-ui-eval.md`; public
  SECURITY/HTTP/PERMISSIONS relatives; public LANGUAGE_MATRIX /
  BENCHMARK_COMPARISON / README / release notes no longer point at missing
  `BENCHMARK.md`.

### Added

- **Group query `--path`** — `projects group query` filters like MCP `path=`;
  ranks non-fixture hits first; JSON `ambiguous` + honesty note when names
  collide across member paths. Still not a shared multi-root sqlite.
- **Nest/Express path= honesty** — Nest gotcha + LANGUAGE_MATRIX rows; sample
  canonical pick prefers Nest `…/src/` and Express `examples/hello*`.
- **Local embeddings (tiny path)** — `scripts/install-local-embed.sh` /
  `codehelper green init-embed`; MCP auto-detects embed URL / green.json /
  `:8766` (opt out `CODEHELPER_EMBED_AUTO=0`). No CI model download —
  [LOCAL_EMBED.md](docs/LOCAL_EMBED.md).
- **Request-flow clustering** — analyze rebuilds `processes`/`clusters` via
  `graph.PersistRequestFlows` (route→controller→service→query spines;
  `layer:*` / `flowfam:*` clusters). See `docs/WORKSPACE_GROUPS.md`.
- **Merged workspace-group snapshot** — `codehelper projects group snapshot <id>`
  aggregates member summaries + cross-repo import→owner edges.
- **Group fan-out query** — `codehelper projects group query <id> <name>` +
  `workspace_groups` on `project_context` (per-member `graph.db`; shared sqlite
  still deferred).
- **GraphQL + event contract hooks** — discovers GraphQL schemas and event
  contracts (AsyncAPI / CloudEvents / event-name patterns) alongside OpenAPI.
  `codehelper projects contracts --cross` / `--group` links shared keys across
  registered sibling repos. See `docs/WORKSPACE_GROUPS.md`.
- **Benchmark bake-off reports** — `scripts/bench-comparison-scaffold.sh` writes
  JSON/MD artifacts with metrics columns plus competitor-class **N/A** rows with
  install hints. Golden fixture under `testdata/bench-comparison/`.
- **`scripts/update-mcpb-hashes.sh`** — refresh `server.json` `fileSha256` from
  local `*.mcpb` (no publish).
- **Product module scaffold** — `CODEHELPER_MODULES` / build tags; see
  [MODULES.md](docs/MODULES.md). Separate packages deferred.
- **Graph densification (parser v12–v13)** — inheritance inbound for
  `CallersOf`/impact; PHP traits + ParentID + use-alias + `$this` recv_type;
  Ruby mixins/`<` + `self.` recv_type; C#/Unity `GetComponent` family; GDScript
  ParentID; Nest `useExisting`/`inject`; Express→`Application`/`Router`; FQCN
  import match.


### Added (3.2 semantic accuracy)

- **Optional LSP rename / implementations** — MCP `lsp` actions `rename` and
  `implementations` (plus existing hover/definition/references);
  `source=lsp` vs `source=graph-fallback`. See [docs/LSP.md](docs/LSP.md).
- **`rename_symbol` / `find_implementations` LSP merge** — when a language
  server is installed, graph plans merge LSP refs/impls (`source=graph+lsp`).
- **C/C++ call edges** — function bodies emit `calls`.
- **PHP `extends` / `implements` edges** — class/interface inheritance for
  inbound impact.
- **Call-edge provenance on context/impact** — `provenance` bands
  (`exact` / `scoped` / `name_only` / `inferred`) on callees and impact nodes.
- **docs/LANGUAGE_MATRIX.md** — LSP policy, GDScript/C++/Ruby honesty, embedding
  hook-only note (no bundled models).

### Added (3.3 architecture intelligence — scaffold)

- **Workspace groups** — `codehelper projects group set|list|show|remove` links
  registered sibling repos; see `docs/WORKSPACE_GROUPS.md`.
- **Import-root detection** — analyze/upsert fills `import_roots` from go.mod /
  package.json / Cargo / Composer / pyproject (preserves `group_ids`).
- **Cross-query CLI** — `codehelper projects cross-query --import …` resolves
  owners via `ResolveCrossRepoEdges` (same-group tagging).
- **OpenAPI contract hook** — `codehelper projects contracts` discovers
  OpenAPI/Swagger path lists.
- **Graph snapshot stub** — `codehelper projects snapshot` writes portable
  summary JSON (`docs/GRAPH_SNAPSHOT.md`); full shared `graph.db` deferred.

### Changed (license — BUSL-1.1)

- **Relicensed Codehelper (and bundled green tools) to Business Source License 1.1.**
  You may use and modify for personal/internal production use without contributing
  back. Competing rebrand / substitute offerings remain restricted until the Change
  Date (**2029-07-24**), when this line converts to **Apache-2.0**. SPDX: `BUSL-1.1`.

### Added (3.1 trust & clarity)

- **SECURITY.md** — vulnerability reporting via VeyrForge / GitHub advisories.
- **docs/PERMISSIONS.md** — read/write/exec/browser/remote capability map + tool profiles.
- **docs/HTTP_SECURITY.md** — loopback-default MCP HTTP; `CODEHELPER_MCP_TOKEN` required off-loopback.
- **docs/LANGUAGE_MATRIX.md** — honest per-language confidence bands.
- **docs/BENCHMARK_COMPARISON.md** — competitor arms, metrics, bed tiers + harness
  scaffold (`scripts/bench-comparison-scaffold.sh`, container outline); no fake numbers.
- **docs/ROADMAP.md** — 3.1 landed vs deferred notes.
- **MCP tool profiles** — `CODEHELPER_TOOL_PROFILE=core|focused|full` (legacy minimal → focused). **Default advertise is `focused` (~12)**; override with `CODEHELPER_TOOL_PROFILE=full` or `CODEHELPER_MINIMAL_TOOLS=off`.
- **Public CI overlays** — CodeQL workflow + release SBOM step + signing notes.

### Added (3.1 release security — supply chain, continued)

- **SBOM in both formats** — release workflow now emits CycloneDX **and** SPDX JSON.
- **Build provenance attestations** — `actions/attest` on every tagged release
  (no stored secret; OIDC only). Verify with `gh attestation verify`.
- **Cosign/Sigstore keyless signing** — `checksums.txt` is signed with
  `cosign sign-blob --yes --bundle` and the Sigstore bundle is published as a
  release asset. No signing key or secret is stored; verify with
  `cosign verify-blob` — see `.github/SIGNING.md`.
- **Approval-metadata fix** — `db_query`/`db_schema`/`ci_status` now correctly
  advertise `openWorldHint=true` (they reach a configured DB host / GitHub
  API), so MCP clients with per-tool consent UI see accurate reach.
- **docs/PERMISSIONS.md audit-trail section** — documents the two local, never-
  transmitted JSONL logs and how to disable each.

### Remaining OPEN / residual ceilings

- Express library hubs denser via example→Application/Router; still example-bound.
- Nest useExisting/inject landed; sample-only monorepo collisions still need path=
  (canonical-pick + group query `--path` help).
- PHP/Ruby inheritance denser; ambiguous-name symref still thinner than Go/TS.
- Godot `_ready` / Unity addon ambiguity at scale (ParentID/GetComponent help).
- Shared verified multi-root `graph.db` still deferred — use `group snapshot` /
  `group query [--path]` fan-out instead.
- Cosign end-to-end signing on the private repo remains deferred until
  `vf publish` (by design — see `.github/SIGNING.md`); wired here on publish.
...
- **Role-aware guidance abstain** — feature/perf guidance no longer reuses
...
Releases from **3.0.3** through **1.1.0** (newest first):

**3.0.3** · **3.0.2** · **3.0.1** · **3.0.0** · **2.59.0** · **2.58.0** · **2.57.0** · **2.56.0** · **2.55.0**
...
- **Orchestration benchmark doc:** internal methodology notes (not shipped in the public tree).
...
### Tested (assistant workflows, live, web-researched)
...
- **Docs:** Agent HTTP API contract for IDE adapters (`codehelper serve`).
...
- **VS Code extension:** Thin UI over the `codehelper` binary (commands for plan/review/verify/finish/expand); developed on a separate branch, not required for CLI/MCP use.
